...
1. **AR-1** — Karvan never holds a model credential, so execution is local and the runtime is
   whatever Node the user already has.
2. **NF6** — `npx karvan up`, no database server, no Docker, no build step at install time.
...
|---|---|---|
| **Exact** | Anything `0.x`; anything whose *output* must be byte-stable (formatters, bundlers); protocol SDKs where a wire change is a correctness bug | `"tsdown": "0.22.14"` |
...
| Package | Version | Pin | Why |
|---|---|---|---|
| `tsdown` | `0.22.14` | exact | Rolldown-based bundler for the one published package; inlines all `@karvan/*` into a single ESM file |
| `vite` | `8.2.0` | `^` | Builds the UI **and** runs in middleware mode inside `karvand` (D10) |
...
No Nx, no moon, no changesets. With exactly one published package (`karvan`), release is
...
| `@modelcontextprotocol/sdk` | `1.30.0` | **exact** | Karvan hosts an MCP stdio server injected via ACP `session/new` (D9) |

These are **spawned, not depended on** — they are the user's own installs and Karvan probes them at
runtime (F3.6). Versions below are what `karvan doctor` saw on 2026-08-02; record them per run, warn
on drift.

| Binary | Package probed | How Karvan speaks to it |
|---|---|---|
...
non-obvious result: the two most important agents do *not* speak ACP themselves. Detail in
...
| `shadcn-vue` | `2.8.1` | devDep, CLI only | Copies component *source* into `src/components/ui/`. ~16 files we own |
...
| `playwright` / `@playwright/test` | `1.62.1` | `^` | ~5 full-stack E2E specs booting a real `karvand` |
...
Two consequences follow, and both are load-bearing for Karvan:
...
Area 2's argument was install ergonomics: zero native compilation means `npx karvan up` never runs
...
- Its API changed *inside* the 24.x LTS line: `createTagStore` in 24.9, `setAuthorizer` in 24.10,
...
//
// The entire surface Karvan is allowed to use. Swapping better-sqlite3 for node:sqlite
...
tarball). Plain `tsc` is not an alternative: it cannot inline `@karvan/*` into one file, and that
...
export default defineConfig({
  ...
  platform: 'node',
  target: 'node24',
  dts: false,                        // nothing imports karvan as a library
  clean: true,
  outDir: 'dist',
...
import type { ApiType } from '@karvan/daemon/http/api.ts'
...
Because `@karvan/daemon`'s `exports` points at `./src/index.ts` in the workspace (see
...
**`z.toJSONSchema()`.** Karvan needs JSON Schema in two places regardless of its validation library:
...
const HandoffContract = z.object({
  findings: z.array(z.object({ file: z.string(), line: z.number().int(), issue: z.string() })),
...
A pty is needed for exactly one thing: **Karvan's own implementation of the ACP `terminal/*` client
methods**, where the agent asks Karvan to run a shell command and build tools change behaviour
...
A silent fallback to `node-gyp rebuild` is a direct threat to `npx karvan up` (NF6) on any user
...
3. **Degrade to a no-TTY spawn if absent.** Wrap the import; if it throws, `terminal/*` runs the
   ...
   `terminal.pty: false`. `karvan doctor` says so out loud.
...
| 9 | **Biome / oxlint / Vitest / tsdown / pnpm official docs** | Their sites returned 403 to the research proxy; versions came from the registry and unpacked tarballs, but *feature and status claims* came from search summaries | Re-read the official docs before committing config |
...
| 11 | **Vendor CLI versions** (§5 table) | Adapters break on flag churn (G7). Codex went 0.107 → 0.146 in roughly a year | `karvan doctor` runs the conformance suite (F3.4) and tells you |
