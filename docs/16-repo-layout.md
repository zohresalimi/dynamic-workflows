...
One pnpm 11 workspace, eight packages plus an `e2e` package, exactly **one** of which is published to npm (D5). No Nx, no Turborepo, no moon at M1 — `pnpm -r run build` and `pnpm --filter @karvan/daemon test` are enough for nine packages and roughly 15k lines. Add `turbo@2.10.8` only when `pnpm -r typecheck` exceeds about 20 seconds locally; it is a drop-in `turbo.json` with `dependsOn`/`outputs` and no code changes.
...
karvan/
  ...
  packages/
    core/        @karvan/core        pure domain + engine logic, zero I/O
    ledger/      @karvan/ledger      SQLite event store, migrations, blob store
    adapters/    @karvan/adapters    ACP client, CLI shims, capability probes
    daemon/      @karvan/daemon      karvand: HTTP+SSE, orchestrator, worktrees, MCP host
    cli/         karvan              THE published package: bins + inlined daemon + built UI
    web/         @karvan/web         Vue 3 + Vite SPA
    testkit/     @karvan/testkit     fake binaries, git fixtures, TestClock, crash harness
    mock-agent/  @karvan/mock-agent  deterministic ACP agent binary (D17)
...
|---|---|---|---|
| `@karvan/core` | `TaskSpec`, `PlanGraph`, `PlanPatch`, `Fact`, `ContextPacket`, the `Event` union, `reduce`, `decide`, patch policy, permission ladder, `Clock`/`Db` port *interfaces* | **nothing** | `zod` only |
| `@karvan/ledger` | Event store, `effect` journal, `plan`/`run`/`node_wake` tables, `PRAGMA user_version` migrations, content-addressed blob store, SSE tail queries | `@karvan/core` | `better-sqlite3@13.0.2` |
| `@karvan/adapters` | ACP client, per-vendor CLI exec shims, capability probing and persistence, golden-recording tee | `@karvan/core` | `@agentclientprotocol/sdk@1.3.0` |
| `@karvan/daemon` | karvand itself: hono HTTP+SSE, orchestrator tick loop, Effect Runner, Planner, Context Builder, Blackboard, Gate Runner, Workspace Manager, MCP host | `@karvan/core`, `@karvan/ledger`, `@karvan/adapters` | `hono`, `@modelcontextprotocol/sdk`, `execa`, `pino`, `@lydell/node-pty` (optional) |
| `karvan` | The npm package. `karvan init/up/run/doctor`, plus the `karvan-mcp` and `karvan-mock-agent` bins. Bundles the daemon and ships the built UI as files | `@karvan/daemon`, `@karvan/mock-agent` | `@lydell/node-pty` (external), everything else inlined |
| `@karvan/web` | Vue 3 SPA, ledger-projection Pinia store, the nine P0 views | `@karvan/core` (**types only**) | `vue`, `pinia`, `@vue-flow/core`, `d3`, `xterm.js`, `shiki` |
| `@karvan/testkit` | Fake agent binaries, hermetic git fixtures, tmpdir fixtures, `TestClock`, `FakeEffectRunner`, crash-fuzz harness | `@karvan/core` | dev-only |
| `@karvan/mock-agent` | A real ACP **agent** binary, seeded and deterministic: scripted chunks, permission requests, fs/terminal callbacks, hang, mid-turn crash, malformed frames, 10 MB line, configurable `agentCapabilities` | **nothing** (deliberately) | `@agentclientprotocol/sdk@1.3.0` |
| `e2e` | ~5 full-stack specs that boot a real karvand on an ephemeral port with fake agents on `PATH` and drive a real browser | `karvan`, `@karvan/testkit` | `@playwright/test` |

`@karvan/mock-agent` deliberately does **not** depend on `@karvan/core`. If it did, a bug in the domain model could be mirrored on both sides of the wire and cancel itself out. It is an independent implementation of the *agent* side of the same published schema, which is what makes it a useful oracle.

---

## 2. Why exactly one package is published

`packages/cli` is `"name": "karvan"`. Every `@karvan/*` package is `"private": true` and is **inlined into the CLI bundle by tsdown** via `noExternal: [/^@karvan\//]`.
...
export default defineConfig({
  ...
  clean: true,
  outDir: 'dist',
  noExternal: [/^@karvan\//],   // inline every workspace package
  external: ['@lydell/node-pty'], // native; must stay a real runtime dep
})
```

This deletes the entire multi-package versioning problem. There are no changesets, no release orchestration, no inter-package semver ranges to keep honest: the release is `npm version patch && pnpm publish`. `@changesets/cli@2.31.1` is alive and fine, but it solves coordination you deliberately do not have. (pnpm 11.13+ also ships native release management — `pnpm change`, `pnpm version -r`, `pnpm lane`, configured under a `versioning:` key — which would be the thing to reach for if `@karvan/*` ever do get published separately. **Unverified**: confirm the exact command surface against pnpm's docs before adopting.)

D17 calls `@karvan/mock-agent` a first-class shipped package. "Shipped" means it ships **inside the `karvan` tarball as a second bin** (`karvan-mock-agent`), not that it is published separately. Users need it for `karvan doctor`, offline demos and reproducing bug reports; that does not require its own npm entry.

**Bundler: `tsdown@0.22.14`** (Rolldown-based, from the Vite/VoidZero org; peer-supports TypeScript ^5/^6/^7; engines `^22.18 || >=24.11`). Pin it exactly — it is still 0.x. `tsup@8.5.1` last published 2025-11-12 and its maintainers now direct new projects to tsdown; treat it as end-of-life. Plain `tsc` cannot inline `@karvan/*` into one file, which is the whole point.
...
```
pnpm --filter @karvan/web build        # -> packages/web/dist
copy packages/web/dist -> packages/cli/dist/ui/
pnpm --filter karvan build             # tsdown, @karvan/* inlined
```

UI assets ship as **plain files** in the tarball, never bundled into JS, and are resolved at runtime with `fileURLToPath(new URL('./ui', import.meta.url))`. `packages/cli/package.json` needs `"files": ["dist"]`; verify the real install with `pnpm pack && cd $(mktemp -d) && npx /path/karvan-0.1.0.tgz up`, plus `publint@0.3.22` and `@arethetypeswrong/cli@0.18.5` in the release script. A missing `files` entry is the classic "works locally, broken on npm" failure.

---

## 3. The `publishConfig` exports-override trick

This is the single biggest day-to-day DX win in the layout. pnpm applies `publishConfig` fields at pack time, so the workspace can resolve `@karvan/core` to **live TypeScript source** while the published tarball resolves it to **built JavaScript**.
...
`@karvan/*` never actually get published, so the `publishConfig` block is belt-and-braces: it exists so that the day one of them does need publishing, or the day someone runs `pnpm pack` on it to inspect the tarball, the result is correct rather than a package pointing at `.ts` files.
...
"dependencies": {
  "@karvan/core":     "workspace:*",
  "@karvan/ledger":   "workspace:*",
  "@karvan/adapters": "workspace:*",
  "hono":             "catalog:",
  "zod":              "catalog:"
}
```
...
> **R1. `@karvan/core` depends on nothing in the workspace, and on nothing that can perform I/O.**
> **R2. Nothing depends on `@karvan/daemon` except `packages/cli`.**

```
──► reads "depends on"

  e2e ──► karvan ──┬──► @karvan/daemon ──┬──► @karvan/adapters ──► @karvan/core
                   │                     ├──► @karvan/ledger   ──► @karvan/core
                   │                     └──────────────────────►  @karvan/core
                   └──► @karvan/mock-agent   ──► (nothing in the workspace)

  @karvan/web      ──► @karvan/core      (type-only imports)
  @karvan/testkit  ──► @karvan/core      (devDependency of every other package)

  @karvan/core     ──► (nothing in the workspace; zod is its only runtime dep)
```

R1 is what makes NF9 ("deterministic core") and the functional-core/imperative-shell split of the [architecture overview](./01-architecture-overview.md#8-functional-core-imperative-shell) structural rather than aspirational. `reduce`, `decide`, the patch policy engine and the permission ladder are pure because `@karvan/core` has no dependency capable of impurity. Time, randomness and ids enter through ports declared in `core` and implemented in `daemon` or `testkit`.
...
it('core imports no node: builtins', async () => {
  const files = await glob('packages/core/src/**/*.ts', { ignore: '**/*.test.ts' })
...
R2 keeps the daemon a leaf. If `@karvan/adapters` ever needs something from `daemon`, that something belongs in `core` (if pure) or is a port that `daemon` implements and injects (if not). The one place this is tested is `e2e`, which depends on the built `karvan` package rather than on `daemon` directly — so the specs exercise the same artefact users install.
...
Consume with `"typescript": "catalog:"`. Exact pins (no caret) for `@agentclientprotocol/sdk` — it went `0.4.5 → 1.3.0` *and* changed npm scope and GitHub org inside about ten months — and for `tsdown`, which is still 0.x.
...
```ts
import { reduce, decide } from '@karvan/core'          // yes
import { reduce } from '@karvan/core/src/reduce.ts'    // no — deep import
```
...
### 7.1 Repo-local `.karvan/` — repo-scoped configuration and human-inspectable exports

Lives inside the target repository being worked on (not inside the Karvan source tree). Follows [PRD §9.4](./prd.md#94-repo-layout).
...
Everything in the committed half is a **team artefact**: it is reviewed in pull requests, it travels with the repo, and it is how a colleague's Karvan behaves the same as yours. Everything in the gitignored half is **per-machine**. `karvan init` writes both halves and appends the gitignored paths to `.gitignore`.
...
### 7.2 Global daemon state — `$XDG_DATA_HOME/karvan`, else `~/.karvan`

```
$XDG_DATA_HOME/karvan/         (falls back to ~/.karvan)
  karvan.lock                  flock target + daemon_epoch (single-instance lease)
...
**This deviates from the sketch in [PRD §9.4](./prd.md#94-repo-layout)**, which shows `ledger.db` under `.karvan/runs/<runId>/`. The deviation is deliberate and is recorded here rather than silently. A per-run database would also put a binary, WAL-journalled SQLite file inside a git repository, which is a bad place for it.
...
`karvan.lock` and the `daemon_epoch` counter are global because the thing they protect against is global — a user running `npx karvan up` in two terminals. Every ledger write carries the epoch and stale-epoch writes are rejected.

On first run, `karvan up` resolves the data dir, runs migrations, probes for installed agent CLIs and records their versions (F3.6), binds port 7777 or the next free one, writes `.karvan/daemon.json` with a freshly generated 32-byte bearer token, and prints the URL.
...
| Branches created by Karvan | `karvan/<runId>__<nodeId>` (flat, D13); integration branches `karvan/int/<runId>` |
| Effect ids | `<runId>/<nodeId>/<attempt>/<ordinal>`, hashed to short hex when embedded in a filename |
| Ports and their fakes | Interface in `@karvan/core` (`Clock`, `Db`), production impl in `daemon`/`ledger`, fake in `testkit` |
| Domain failures | Values in a closed union, never thrown; `throw` is reserved for programmer bugs |
| Env vars | `KARVAN_*` prefix. `KARVAN_DEV`, `KARVAN_KEEP_TMP`, `KARVAN_RECORD`, `KARVAN_FAKE_SCENARIO` |

Root scripts:

```jsonc
"scripts": {
  "dev": "KARVAN_DEV=1 node --watch --watch-path=packages --env-file-if-exists=.env packages/daemon/src/main.ts",
  "build": "pnpm --filter @karvan/web build && pnpm --filter karvan build",
  "typecheck": "tsc -b && pnpm --filter @karvan/web exec vue-tsc --noEmit",
  "test": "vitest run",
...
- **Do not publish `@karvan/*`.** The moment a second package is published you have inter-package semver, a release order, and a reason for changesets — all of which the single-package design exists to avoid.
- **Do not let `@karvan/core` acquire an I/O dependency.** The purity test in §4 is cheap; add it before the first temptation, not after.
