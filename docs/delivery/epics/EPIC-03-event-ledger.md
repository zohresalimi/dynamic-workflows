...
At the end of this epic `DeFlowd` has a durable, append-only ledger it can be killed with `SIGKILL`
...
`seq` is the identity of an event _outside_ the database — the SSE frame id a browser tab persisted,
...
And two `DeFlowd` processes reducing the same ledger both derive the same ready set, both spawn the
same agent, both burn tokens and both commit to the same branch — SQLite protects the _database_,
not the _scheduler_.
...
- `flock` on `<dataDir>/DeFlow.lock` and the `daemon_epoch` counter stamped on every write.
...
- `decide()`, the scheduler tick loop, the effect journal _protocol_ (`durable()`, `reconcile()`),
...
- Orphaned-child reaping on boot — [EPIC-05](./EPIC-05-provider-adapters.md), KAR-05.9. This epic
  makes the `(pid, process_start_time)` journal _storable_; the reaper is elsewhere.

## Definition of Ready (epic level)

- [ ] EPIC-02 Done: the `Event` union, envelope, upcaster registry and `NodeFailure` exist in
      `@DeFlow/core`, and `pnpm schemas:check` is green.
...
- [ ] `test/fixtures/` has the tmpdir fixture with `DeFlow_KEEP_TMP` support and the
...
- [ ] The **crash-fuzz suite runs in CI on every push** (`pnpm vitest run --project crash-fuzz`)
      with `DeFlow_KEEP_TMP=1` and `actions/upload-artifact` on failure, on `ubuntu-26.04` and
...
and decision D6. The `Db` interface lives in `@DeFlow/core` (which cannot perform I/O); the
`better-sqlite3@13.0.2` implementation lives in `@DeFlow/ledger`; the fake lives in
`@DeFlow/testkit`. The port is deliberately tiny — `prepare`, `exec`, `transaction`, `close`,
...
| 5   | integration | Assert `foreign_keys` is `ON` after a _failed_ run                                          | The restore is in the happy path only    |
...
2. `reduce()` has no read path to `io_chunk`: the reducer's input type is `Event`, the `Db` handle
   is not in scope inside `@DeFlow/core`, and an architecture test asserts `packages/core` does not
   import `@DeFlow/ledger`.
...
| 2   | unit        | Architecture test: `packages/core/package.json` dependencies contain no `@DeFlow/ledger`                                      | The reducer gained a db handle                    |
...
Ships `RunState` and `reduce(state: RunState, event: Event): RunState` in `@DeFlow/core`: no I/O, no
...
3. `reduce` performs no I/O and reads no clock: an architecture test asserts `@DeFlow/core` has no
...
6. Checkpointing is disable-able with an env flag (`DeFlow_NO_CHECKPOINT=1`) and the full suite
...
| 5   | integration | Overwrite `state_json` with a _valid_ but wrong-shaped object; assert it is rejected                   | Only JSON parse errors are caught                 |
| 6   | integration | Full suite run with `DeFlow_NO_CHECKPOINT=1`                                                           | Something started depending on the cache          |
...
`<dataDir>/DeFlow.lock`**, taken at boot, makes the second instance fail fast with a message naming
the pid holding the lock — not a stack trace. **`daemon_epoch`**, a counter bumped on every
`DeFlowd` start and stamped on every write, means that if a second daemon somehow does start, its
...
1. The first `DeFlowd` acquires an exclusive `flock` on `<dataDir>/DeFlow.lock` and holds it for its
   lifetime.
2. A second `DeFlowd` process against the same data directory exits non-zero within 1 second with a
   message of the form `DeFlowd is already running (pid <n>) — data dir <path>`. No stack trace, no
...
| 6   | e2e         | Two real `DeFlowd` processes on the same data dir; assert exactly one binds the port and one exits cleanly         | The daemon partially initialises before locking |
...
4. An older binary replaying a ledger containing newer event kinds produces a **degraded but
   ...
   starts and serves.
5. The crash-fuzz suite runs N iterations of: start `DeFlowd` with mock agents on `PATH` and a
   scripted multi-node run; sleep a seeded random interval; `kill -9` the daemon; restart over the
   same `.DeFlow/`; then assert (a) no effect executed twice, checked against the effect journal
...
### KAR-03.9 — Content-addressed blob spill for oversized payloads _(added)_
...
blob store to `@DeFlow/ledger` at `$XDG_DATA_HOME/DeFlow/blobs/<ab>/<sha256>`. No skeleton story
...
KAR-03.4 because the reducer is pure and needs only `@DeFlow/core` — this is the story to work on
