...
At the end of this epic Karvan is a thing a person installs and runs, rather than a repository they
clone. `npx karvan init` bootstraps a target repo, `npx karvan up` brings the daemon to a browser in
under three seconds, `npx karvan run "…"` drives a complete run to a terminal without a browser at
all, and `npx karvan doctor` tells the truth about what this specific machine can and cannot do
...
**NF6 is the adoption argument.** "Single-binary-ish install: `npx karvan up`. No database server,
no Docker requirement for the core." M2's definition of done is *a colleague installs it unaided and
finishes a real task* — and the author is the first test subject. Everything in the repo layout that
looks like fussiness (exactly one published package, `noExternal: [/^@karvan\//]`, UI assets as
...
never hardcoded — *"a hardcoded matrix is wrong within a month"*, and the research's own snapshot
...
subscription auth**, whose failure mode is *"you thought you were on your subscription and you were
being billed"*. Neither condition raises an error anywhere else in the system. If `doctor` does not
catch them, nothing does.

**The tarball is a different program from the workspace.** In development `@karvan/core` resolves to
...
monorepo and fatal outside it — *"a missing `files` entry that drops `dist/ui/`, so the daemon starts
and serves a blank page"* is the named classic. KAR-18.6 exists because that class of bug is only
...
- `karvan init`: the committed half of `.karvan/` (`config.yaml`, `gates/`, `templates/`, `memory/`,
  ...
  committed config.
- `karvan up`: the eight-step boot sequence of [03 §9](../../03-local-development.md) — resolve
  directories, open and migrate the ledger behind a `VACUUM INTO` backup, take the `flock` on
  `<dataDir>/karvan.lock` and bump `daemon_epoch`, probe providers from cache, pick port 7777 or the
  next free one, generate the 32-byte token, write `.karvan/daemon.json` at mode `0600`, bind
  `127.0.0.1` and print `http://127.0.0.1:<port>/#token=<token>` — plus graceful shutdown, orphan
  reaping and the NF3 cold-start budget.
- `karvan run`: headless execution against the same daemon, consuming `hc<ApiType>` and
...
- `karvan doctor`: runtime, git, sandbox prerequisites, installed agents and versions, the
...
- Packaging: the fixed build order, `tsdown@0.22.14` with `@karvan/*` inlined and
...
- Clean-room install verification: `pnpm pack`, `mktemp -d`, `git init -b main`,
  `npx /path/karvan-0.1.0.tgz up`, a real mock-agent run through the installed bytes, on macOS and
  Linux.
- `karvan ledger snapshot` and `karvan status` — the two diagnostics that make a solo builder's
  post-mortems possible (added story, see KAR-18.7).

**Out of scope:**

- **`karvan replay`.** The replay daemon mode is [EPIC-16](./EPIC-16-ui-foundation.md) KAR-16.5; this
...
- **The HTTP surface itself** — routes, the SSE frame contract, bearer auth and origin validation are
  [EPIC-15](./EPIC-15-daemon-api.md). This epic is a *client* of them, and the producer of the token
...
- **Windows.** NF5 puts it at M3. `karvan doctor` must *say so* rather than fail obscurely, and
...
- [ ] EPIC-04 is Done: `karvan-mock-agent` is a real bin, so every scenario in the flow file can run
...
- [ ] A `karvan-0.0.0` version number and an npm package name are settled well enough to pack. (The
...
- [ ] `karvan doctor` on a machine with **no agent CLI installed** exits 0, names what to install,
...
- [ ] Cold start measured on the author's own laptop is under 3 s (NF3), reported by `karvan up
...
### KAR-18.1 — `karvan init` workspace bootstrap
...
**As** an engineer adopting Karvan on a repository, **I want** one command that makes the repo
Karvan-aware without polluting it, **so that** the committed half of the configuration is reviewable
in a pull request and the machine-local half never lands in git.

`.karvan/` has two halves and [16 §7.1](../../16-repo-layout.md) is explicit that they are not
interchangeable. `config.yaml`, `gates/`, `templates/`, `memory/` and `.worktreeinclude` are **team
artefacts**: they travel with the repo and they are how a colleague's Karvan behaves the same as
...
detection runs here (PRD §6.3: *"detect providers, create `.karvan/`"*) but its result belongs in the
**global** probe cache under `$XDG_DATA_HOME/karvan`, not in the committed config — a committed
capability list is exactly the hardcoded matrix AR-5 forbids.

**Acceptance criteria**

1. In a git repository with no `.karvan/`, `karvan init` creates `config.yaml`, `gates/`,
   `templates/`, `memory/` and `.worktreeinclude`, and appends `.karvan/daemon.json`,
   `.karvan/wt/` and `.karvan/runs/` to the repository's `.gitignore`.
2. The generated `config.yaml` validates against the emitted JSON Schema in `.karvan/schemas/`
   (EPIC-02 KAR-02.8) and contains no provider capability claims — only provider *selection* and
   policy.
...
4. Outside a git working tree the command refuses with
   `karvan init: not inside a git working tree (run 'git init' first)` and exits 5. It does not
   create a partial `.karvan/`.
5. `init` resolves and creates the global state directory (`$XDG_DATA_HOME/karvan`, else
   `~/.karvan`) and reports its absolute path. If it is not writable, the failure names the exact
...
| 4 | integration | `init` in a plain `mkdtemp` directory with no `.git` | It creates `.karvan/` anyway and exits 0 |
...
### KAR-18.2 — `karvan up` daemon lifecycle
...
**As** an engineer, **I want** `karvan up` to bring the daemon to a browser tab in one step and to
...
one: a second `npx karvan up` in another terminal is described in the architecture as *"very
common"*, and two schedulers over one SQLite file will interleave effect execution even though
SQLite itself enforces one writer. The `flock` on `<dataDir>/karvan.lock` plus the `daemon_epoch`
...
1. `karvan up` in an initialised repo binds `127.0.0.1`, prints
   `http://127.0.0.1:<port>/#token=<token>`, writes `.karvan/daemon.json` as
...
2. Cold start is under 3 s (NF3) on the author's machine with a warm probe cache;
   `karvan up --timings` prints per-step milliseconds for the eight steps so the budget is
   measurable rather than asserted.
3. A second `karvan up` while a live daemon holds the lease exits 2 with
   `karvan up: another karvand is already running (pid <pid>, port <port>) — open <url> or run 'karvan status'`.
...
4. If port 7777 is occupied, the daemon binds the next free port via `get-port`, and the printed
   URL, `daemon.json` and `karvan status` all agree on it. With an explicit `--port` that is
...
7. On SIGINT/SIGTERM the daemon releases the `flock`, removes `.karvan/daemon.json`, and terminates
   its child process group; a subsequent `karvan up` starts cleanly with no manual intervention.
8. On a machine with no agent CLI installed, `karvan up` still starts, reports zero available
   providers with install hints, and the daemon refuses to *schedule* agent nodes rather than
   failing at boot (NF7).
9. `karvan up` reads no credential file and captures no auth-command output (AR-1); the assertion is
...
| # | Level | Test | Red when |
|---|---|---|---|
| 1 | e2e | Boot a real `karvand` on an ephemeral port in a `mkdtemp` repo with the mock agent on a temp `PATH`; `GET /api/health` answers unauthenticated and every other route 401s | Boot completes before the token file is written, so the CLI's own first authenticated request races it |
...
### KAR-18.3 — `karvan run` headless execution
...
exists to record a fixture from before any view is built, and so that Karvan is usable over SSH and
...
`karvan run "…"` consumes the identical stream through the identical
...
1. `karvan run "<task>"` with no daemon running autostarts `karvand` detached, waits on
   `GET /api/health`, creates the run, and streams node lifecycle to the terminal until a terminal
   state.
2. With a daemon already up, `karvan run` attaches to it — it never starts a second daemon and never
   trips the lease.
3. The first Ctrl-C **detaches**: it prints
   `detached — run <runId> continues; 'karvan run --attach <runId>' to watch, 'karvan cancel <runId>' to stop`
...
5. If the stream drops mid-run, the CLI reconnects with `?since=<lastSeq>` and the rendered
   ...
   *strictly greater than `seq`*, never `seq + 1`).
...
| # | Level | Test | Red when |
|---|---|---|---|
| 1 | e2e | With no daemon, run a scripted 4-node mock plan to completion through `karvan run`; assert exit 0 and the rendered transcript against a file snapshot through the normalising serializer | The CLI polls `/api/health` with the bearer header before the token file exists and 401s itself |
| 2 | e2e | Start a daemon, then `karvan run`; assert `daemon_epoch` did not change and only one pid holds the lease | Autostart is unconditional |
...
| 7 | integration | `karvan run --json` with stdout piped to a file; assert every line parses and no line contains `[` | The renderer detects TTY once at import and colours the JSON path |
...
work: *"Do not start W11 until at least one full run completes headlessly."* The fixtures in
...
### KAR-18.4 — `karvan doctor` environment and capability probe
...
- **Runtime** — Node `>= 24`, pnpm version, a writable global state dir
  (`$XDG_DATA_HOME/karvan` or `~/.karvan`) and a writable repo-local `.karvan/`.
...
- **Sandboxing** — Linux: `bwrap` and `socat` on PATH, and
  ...
  permission levels that are *honourable on this machine*.
- **Agents** — which vendor binaries are installed, their **absolute resolved paths** (karvand's
...
2. On a machine with **no agent CLI installed at all**, `doctor` exits 0, reports zero agents, and
   ...
   development and `karvan replay` work regardless. It never throws and never prints a stack trace.
...
4. On Linux, a missing `bwrap` or `socat`, or
   ...
   `read` only, names `/etc/apparmor.d/bwrap` as the fix, and states that Karvan sets
...
8. An unwritable global state dir or repo-local `.karvan/` is a `fail` naming the absolute path and
   the errno, exit 5.
...
[16 §2](../../16-repo-layout.md) settles the mechanism. `packages/cli` is `"name": "karvan"`; every
`@karvan/*` package is private and inlined by `tsdown@0.22.14` through `noExternal: [/^@karvan\//]`,
with `external: ['@lydell/node-pty']` because it is native and must stay a real runtime dependency.
The build order is fixed and load-bearing: `pnpm --filter @karvan/web build` → `packages/web/dist` →
copied to `packages/cli/dist/ui/` → `pnpm --filter karvan build`. UI assets ship as **plain files**,
...
2. The bundle contains no `@karvan/` import specifier and no `.ts` import specifier; the only
...
3. `packages/cli/package.json` declares `"files": ["dist"]`, `"type": "module"`,
   `engines.node >= 24`, and bins for `karvan`, `karvan-mcp` and `karvan-mock-agent`. There is no
   dual CJS build.
...
| 2 | integration | Static scan of `dist/*.mjs` for `@karvan/` and `.ts` specifiers, and for exactly one external `require`/`import` of `@lydell/node-pty` | `noExternal` was written as a string rather than a regexp and matches nothing |
| 3 | unit | Parse `packages/cli/package.json`; assert `files`, `bin`, `type`, `engines`, and that no `@karvan/*` appears in `dependencies` | A workspace package leaks into `dependencies` and npm tries to fetch a private name |
...
| 5 | unit | Config guard: no `paths` key in any `tsconfig*.json`; no import matching `@karvan/[^'"]+/` | Someone adds `@/` for convenience; dev is green and the tarball is broken |
...
`karvan-0.1.0.tgz`, then `cd "$(mktemp -d)"`, `git init -b main demo && cd demo`, and
`npx /path/to/karvan-0.1.0.tgz up` — *the exact bytes a user would get*. What it catches that
`pnpm dev` cannot: a missing or wrong `files` array; a lost shebang or exec bit; an `@karvan/*`
...
2. In the same clean room, with `karvan-mock-agent` from the installed tarball on `PATH`, a scripted
...
7. The clean room is removed on success and preserved under `KARVAN_KEEP_TMP=1` on failure, with the
...
| 2 | e2e | A scripted mock run through the installed `npx <tgz> run`, with the tarball's own `karvan-mock-agent` symlinked onto the temp `PATH` | `mock-agent.mjs` was never added to the tsdown `entry` array, so the second bin does not exist in the tarball |
...
### KAR-18.7 — Diagnostics: `karvan status` and `karvan ledger snapshot` *(added)*
...
skeleton's six stories. `karvan ledger snapshot <runId> --out <path>` is `VACUUM INTO` behind a
...
That is NF8 made operational. `karvan status` answers the question the troubleshooting table in
[03 §13](../../03-local-development.md) sends people to `.karvan/daemon.json` for, and it is the
command the lease-refusal message in KAR-18.2 points at.

**Acceptance criteria**

1. `karvan ledger snapshot <runId> --out <path>` writes a single SQLite file; `PRAGMA
...
2. The snapshot is queryable with the documented one-liners — `SELECT seq, kind, node_id, attempt
   FROM event WHERE run_id='<id>' ORDER BY seq LIMIT 50` returns rows — without Karvan installed.
3. `karvan status` reports the live daemon's pid, port, `daemon_epoch`, uptime, and a one-line
...
| 2 | integration | `SIGKILL` a booted daemon, then `karvan status`; assert `stale` and the path, and that no process was signalled | `status` trusts the pid and reports a live daemon that is gone — or worse, matches a recycled pid |
| 3 | integration | `karvan status` in a repo with no `daemon.json`; assert exit 0 and the message | It exits non-zero, so a shell script wrapping it breaks on the normal case |
...
| R3 | **A5-2 / A5-3: sandbox prerequisites are platform-specific and fail open.** macOS 26 Tahoe broke Seatbelt profiles in practice and whether Claude Code 2.1.220 fixes it is unverified; Ubuntu 24.04+ AppArmor blocks bubblewrap silently. | `doctor` reports *honourable levels*, not "sandbox: ok". EPIC-08 sets `failIfUnavailable: true` and `allowUnsandboxedCommands: false` for every non-`full` level so the ladder fails closed; this epic's job is to say so before a run starts. |
...
| R5 | **The tarball can regress between releases in ways only a clean room sees.** | KAR-18.6 is a CI job on every release tag plus a per-milestone manual run, and criterion 5 asserts the *verifier itself fails* on a deliberately broken tarball — a green verifier that cannot go red is worse than none. |
| R6 | **`npx karvan up` in two terminals is described in the architecture as "very common"**, and the refusal path is the first thing an operator meets when something is wrong. | KAR-18.2 criterion 3 fixes the exact sentence, including the live pid and port, and points at `karvan status`. The lease is EPIC-03's; the sentence is this epic's. |
