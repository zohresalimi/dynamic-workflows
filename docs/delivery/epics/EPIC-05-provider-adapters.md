...
At the end of this epic `@DeFlow/adapters` turns heterogeneous vendor binaries into **one internal event
vocabulary**, without ever holding a credential. DeFlow is an ACP client that can `initialize`,
...
This is the highest-churn layer in DeFlow and the one where research most sharply contradicted the PRD.
...
And without KAR-05.4, DeFlowd OOMs. That is not theoretical: the SDK's `LineBuffer` has no maximum line
length, DeFlowd is a long-lived daemon supervising runs for days, and a trivial `claude -p "say ok"` turn
...
- [ ] EPIC-02 has emitted `.DeFlow/schemas/` and the `NodeFailureReason` union, so adapter failures map onto
...
**As** DeFlow, **I want** to drive any ACP-speaking agent through one client implementation, **so that**
five vendors cost one integration instead of five, and every file access and command execution passes
through DeFlow on the way.
...
backpressure (DeFlow does not request the next frame until it has finished with the current one, the OS
...
4. Each `session/update` notification is translated into a DeFlow event and **durably appended before**
...
on this machine, **so that** DeFlow never schedules a node onto an adapter that cannot honour it, and never
believes a documentation claim over a handshake.

DeFlow already performs `initialize`. Persist its **full, unmodified** response:
...
**As** DeFlow, **I want** each vendor's correct invocation encoded once in a registry, **so that** the
...
but `--help` marks it _"(deprecated, use --acp instead)"_: use `--acp` and fall back only if argv parsing
...
7. Copilot's `authMethods` entry carrying `_meta["terminal-auth"]` is rendered as a command for the user;
   DeFlow **never runs it and never captures its output** (AR-1, NF2), asserted by a test that no adapter
...
| 7   | unit        | Feed a Copilot-shaped `authMethods` with `_meta['terminal-auth']`; assert the result is a rendered string and that no spawn occurred | DeFlow tries to be helpful and runs the login command |
...
**As** the operator of a daemon that supervises runs for days, **I want** DeFlowd to survive an agent that
...
result — grows that array until DeFlowd OOMs. Measured environment defaults on the same machine, 2026-08-02:
...
3. **Spill payloads over ~256 KiB** to `~/.DeFlow/blobs/<sha256[0:2]>/<sha256>`, storing only
...
**As** DeFlow, **I want** two resume strategies behind one interface selected from the probed capability
...
The design rule this implements is unambiguous: **DeFlow's own SQLite ledger is the sole source of truth
for a run. Every prompt DeFlow sends must be reconstructible from that log alone. `session/resume` is a
...
6. `session/load` is never used as a substitute for resume. If it is used deliberately, deduplication is
   keyed on DeFlow's own event ids and a test proves no duplicate ledger row results.
...
**As** an agent working on a DeFlow node, **I want** workflow-level tools — read a `Fact`, pull an artifact
...
tools **without touching the user's global MCP config**, which DeFlow must never mutate.
...
`npx deflowai up` (NF6). Deep subpath imports keep it from _loading_; if install size becomes a real problem,
...
1. `DeFlow_RECORD=1` tees raw stdout, stderr, exit code and inter-chunk timing to
...
7. Both layers of assertion run on each recording: raw frames for conformance, plus a snapshot of the
   **normalised** DeFlow event vocabulary through the normalising serializer. Snapshotting only the
...
| 6   | integration | `DeFlow_RECORD=1` on a mock run; assert the ndjson file exists with both directions and monotonic `t` offsets                                                                        | The tee is in the adapter and only records `out` |
...
_(scope-cut candidate — see [adapter layer §8.5](../../07-provider-adapter-layer.md) and
[roadmap](../../17-roadmap.md))_

**As** DeFlow, **I want** a documented, permanently-retained fallback that drives a vendor CLI by flags and
...
The shim gives up ACP's mediation — DeFlow no longer sits in the path of every file access and command
...
cannot express the requested level, DeFlow **refuses to schedule** (`safety.permission-unschedulable`)
...
leaves an orphan behind, **so that** the kill switch is real and a crashed DeFlowd does not leave agents
...
`detached: false`, a bash script that backgrounds two children put the grandchildren's PGID in **DeFlowd's
own process group** — `child.kill('SIGTERM')` killed only bash, both grandchildren kept running in state `S`,
and you cannot group-kill because signalling that group would kill DeFlowd itself. With `detached: true`,
...
`detached: true` means **the agent survives DeFlowd's death**, which is not optional to handle. Persist
...
| #   | Level       | Test                                                                                                                                                                      | Red when                                                           |
| --- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------ |
| 1   | integration | Spawn `bash -c 'sleep 300 & sleep 300 & sleep 300; wait'` detached; read `ps -eo pid,pgid,stat`; assert all four share `pgid === child.pid`                               | `detached` is false and pgid is DeFlowd's                          |
...
**As** an engineer whose employer supplies a company API key, **I want** DeFlow to drive a provider through
its HTTP API when I explicitly hand it a credential, **so that** I can use DeFlow where no vendor CLI is
installed or authenticated, and so that provider-neutrality means _provider_-neutral rather than
_CLI_-neutral.
...
**AR-1 is not weakened by this story and the acceptance criteria enforce that.** AR-1 forbids DeFlow
_possessing_ a credential it was not explicitly given. A key the operator hands over in an explicit,
per-provider opt-in is a different thing from DeFlow reading a vendor's token file — and the distinction
must be visible in the product, not just in this paragraph.

**Acceptance criteria**

1. The adapter is inert unless the operator opts in explicitly per provider in `.DeFlow/config.yaml`.
...
| **MCP SDK weight slows `npx deflowai up` (NF6).**                                                                                                                                                                                        | Medium                  | Deep-subpath imports only, asserted by a repo grep. Vendoring a ~200-line stdio-only server is the escape hatch.                                                                                                                                                                                                                                                                        |
