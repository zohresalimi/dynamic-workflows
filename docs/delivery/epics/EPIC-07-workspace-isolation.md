...
At the end of this epic Karvan can put N write-capable agents on one repository at the same time
...
Worktrees are the reason parallel agent work is possible at all, and the PRD rates *"no workspace
isolation"* (G5) as **High**: two write-capable agents in one working directory race on files and
lockfiles within minutes. But the category research is equally clear that creating a worktree is
the easy half — *"every worktree tool creates worktrees; almost none of them make the worktree
work"* ([§5](../../09-workspace-and-safety.md)). A worktree with no `node_modules` and no `.env`
is a worktree the agent's very first command fails in.

The sharper reason is that this epic is where the *verified footguns* live. Git will not check out
...
exists to prevent; and the PRD's own `karvan/<run-id>/<node-id>` branch scheme cannot coexist with
...
Finally, the `merge-tree` conflict matrix (D14) is the one piece of design in Karvan the research
...
overlapping declared scopes" to "start in parallel, serialize on the first *detected* conflict".
...
- Flat branch naming (**D13**): `karvan/<runId>__<nodeId>` and `karvan/int/<runId>`, with
...
- Branch-occupancy pre-check by scanning the porcelain list, so Karvan never parses a git error
  string to detect the collision — and Karvan's own message when the occupant is the operator's
  main checkout.
- The dirty-removal salvage sequence: `status --porcelain=v2 -z` → `workspace.dirty_on_remove`
  event → `karvan: WIP salvage` commit on the node branch → `remove --force`.
...
- The scheduler signal derived from that table: demote the later-starting node to blocked on the
  first `clean = 0`.
- The integration loop: create `karvan/int/<runId>` at run start, merge lowest-conflict-count
...
- Scheduling policy, the ready set, semaphores and the effect journal —
  [EPIC-06](./EPIC-06-orchestrator.md). This epic supplies the `git` effect's *implementation* and
...
- [ ] **EPIC-06 Done through KAR-06.3 and KAR-06.4.** Git operations are journaled effects; the
      `Karvan-Effect-Id` trailer convention and "already exists is a success" reconciliation rule
...
- [ ] `KARVAN_KEEP_TMP` is honoured by the tmpdir fixture, because a failed worktree is
...
- [ ] No test in this epic uses `isomorphic-git`, `simple-git`, `memfs`, or a mocked `spawn`. A grep
      in CI enforces it.
- [ ] The regression test that the PRD's `karvan/<runId>/<nodeId>` scheme fails is green, so nobody
      reintroduces it.
...
*conflict*, a normal expected result, and an exception-throwing wrapper destroys that signal before
...
4. Every argument shape in the F5.5 refusal set throws: `push` to the local default branch, `push`
   ...
   `update-ref` targeting the default branch. `push --force-with-lease` on a `karvan/` ref is
   allowed.
...
6. With `git --version` reporting 2.37.x, `karvan doctor` fails hard and the daemon refuses to
   start a run, naming `merge-tree --write-tree` as the missing capability. At 2.43 it warns and
   proceeds.
7. `gitChildEnv()` keeps `SSH_AUTH_SOCK` — the `Git` wrapper is the only thing in Karvan that
...
**Notes / risks** — `execa` is used here and *only* here. Agent processes use raw
...
create-then-lock sequence races with Karvan's own background reaper across a daemon restart, and
...
1. A write node's worktree is created by exactly
   `worktree add --lock --reason "karvan run=<runId> node=<nodeId>" -b karvan/<runId>__<nodeId> <path> <baseRef>`
...
3. Attempting to create a worktree on a branch already listed in the porcelain output is refused by
   Karvan **before** `git worktree add` runs, with a `workspace.branch_occupied` event naming the
   occupying path.
...
| 3 | integration | Second `add` on the same branch: Karvan refuses pre-flight; the spawn log shows no `worktree add` | Occupancy pre-check is missing |
...
| 5 | integration | `add <path> <mainBranch>` where `<mainBranch>` is the main checkout's branch → Karvan's own message, and the ledger event names `<mainRepo>` as occupant | The main checkout is not treated as an occupant |
...
flat `karvan/<runId>__<nodeId>` scheme, **so that** a run-level integration branch is possible and
no generated id can reach git as a flag or an invalid ref.

**Decision D13** overrides the PRD. `karvan/<runId>/<nodeId>` cannot coexist with
`karvan/<runId>`, because git's refs storage cannot have both a file and a directory at
`refs/heads/karvan/r1`. Verified on git 2.43: after `karvan/r1/n1` exists,
`git branch karvan/r1` fails with
`fatal: cannot lock ref 'refs/heads/karvan/r1': 'refs/heads/karvan/r1/n1' exists; cannot create 'refs/heads/karvan/r1'`,
...
scheme *fails*, not merely that the new one works. Validation is two-layer: `BRANCH_SAFE =
...
1. `nodeBranch(runId, nodeId)` returns `karvan/<runId>__<nodeId>`; `integrationBranch(runId)`
   returns `karvan/int/<runId>`; neither contains a slash beyond the two literal ones.
...
7. A regression test creates `karvan/r1/n1` on a real repo and asserts `git branch karvan/r1` fails
   with the refs lock message — documenting *why* D13 exists.

**Test plan (TDD)**

| # | Level | Test | Red when |
|---|---|---|---|
| 1 | unit | `nodeBranch('r1','n1') === 'karvan/r1__n1'` | Hierarchical scheme still in place |
...
| 4 | integration | **Regression:** `git branch karvan/r1` after `karvan/r1/n1` exists fails with `cannot lock ref` | Someone reintroduces the PRD scheme |
| 5 | integration | `git branch --list 'karvan/int/*'` after run start shows exactly one integration branch, and creating a node branch under the same run does not disturb it | Integration prefix collides with node names |
...
`karvan: WIP salvage` commit, and only then `worktree remove --force`. The ordering is the whole
point: force becomes acceptable only *after* the salvage commit is durable. The companion verified
...
2. The salvage commit lands on the node's own branch with subject `karvan: WIP salvage`, includes
   untracked files, and its OID is recorded in a `workspace.wip_salvaged` event.
3. After removal, `git log karvan/<runId>__<nodeId>` shows the salvage commit and `git show` of it
...
6. A detached read-node worktree that is somehow dirty is salvaged to a throwaway
   `karvan/salvage/<runId>__<nodeId>` branch rather than being force-removed, because a detached
...
**Layer 3** is `workspace.setup` in `karvan.config.ts` with a success marker cached on the hash of
...
target is the *store*. And the reflink fast path (`cp --reflink=auto`, APFS `clonefile`) is
...
research, so its specific config keys are unconfirmed. The store-sharing *mechanism* is
...
4. After a write node commits, probes run against `karvan/int/<runId>` and every other in-flight
...
5. A probe row whose stored tips no longer match the current tips is treated as stale and re-probed
   rather than read.
6. On the first `clean = 0` between two in-flight write nodes, the later-*starting* node is demoted
...
**Notes / risks** — *Apparently novel, stated honestly.* No shipping orchestrator found in the
...
[§7](../../09-workspace-and-safety.md). `karvan/int/<runId>` is created from the run's base ref at
...
the first merge. On conflict, Karvan spawns a resolution node at `worktree` level on the
...
1. `karvan/int/<runId>` is created from the run's base ref during run start, before the first node
...
7. The integration branch is never pushed to the default branch by Karvan; the run's output is the
...
| 6 | integration | `git log --merges karvan/int/<runId>` shows one commit per node branch with `runId`/`nodeId` in the message | Fast-forward merges collapse the history |
...
that** a `kill -9` of karvand does not leave a repository littered with locked worktrees that
`prune` will not touch.

`detached: true` means an agent survives karvand's death — that is the price of group-kill, and
...
| # | Level | Test | Red when |
|---|---|---|---|
| 1 | integration | Spawn a detached `bash -c 'sleep 300'`, record its row, kill karvand, restart → the process is gone and the worktree unlocked and pruned | Reaping missing or ordered wrong |
...
| **Submodules in linked worktrees are untested (A5-8).** Git's submodule support in worktrees has historically been incomplete, since submodule state partly lives in the main `.git`. | Medium | Not a story, deliberately. If any target repo uses submodules, prototype it *before* KAR-07.2 lands, because it is a plausible source of silent breakage with no verified answer. Otherwise carry it as a known gap. |
...
| **Repository git hooks run on checkout and commit inside a worktree**, and hooks are shared from the main repo. Whether Karvan's own orchestration git commands should run with hooks disabled is an open question ([security model §6](../../15-security-model.md)). | Medium | Spike it before KAR-07.4 lands — the salvage commit is the first place Karvan's own git call could trigger a repository-authored hook. Disabling hooks may break legitimate repo workflows, so the answer is not obviously "disable". |
| **F5.3 is deliberately downgraded.** Declared path scopes become a plan-time prediction, not the enforcement mechanism. | Low | Stated explicitly in D14 and reflected in KAR-07.6 AC 7 and EPIC-08 KAR-08.7 — the requirement is covered, but as a *warning* surface plus request-time rejection via `ToolCallLocation.path`, which is strictly better than the PRD's completion-time diffing. |
