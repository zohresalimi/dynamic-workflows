...
At the end of this epic there is **one pure function in DeFlow's own code** that decides whether any
...
one DeFlow spawned — by one control that verifies its own success correctly.

## Why this matters

The PRD rates ODW's binary permission model (G6) as **High** and lists _"destructive action at the
execution boundary"_ as a **High** risk with the Kiro/AWS incident as its reference failure. The
...
The second reason this epic is worth its size is that ACP makes it cheap. DeFlow is the ACP
_client_, so it implements `session/request_permission`, `fs/read_text_file`, `fs/write_text_file`,
`terminal/create`, `terminal/output`, `terminal/wait_for_exit`, `terminal/kill` and
`terminal/release`. **DeFlow sits in the path of every file access and every command execution, for
...
- The policy function: `decidePermission(level, request, scope) -> allow | deny | gate`, pure, in
  `@DeFlow/core`, covering all four levels (`read`, `worktree`, `worktree+net`, `full`) against
  `fs/write_text_file`, `fs/read_text_file`, `terminal/create` and network.
- The `mediatedExecution: true | false` capability bit as the _only_ refuse-to-schedule rule — the
...
- `RequestPermissionOutcome` handling including `{outcome: "cancelled"}` as a first-class outcome,
  and the four `PermissionOptionKind` values DeFlow offers back.
...
- Request-time rejection via `ToolCallLocation.path` — the improvement over F5.3's completion-time
  detection.
- The default-deny command allowlist sourced per repo from `DeFlow.config.ts`, plus the cheap
...
- `buildChildEnv()` in `packages/proc/src/env.ts` — allowlist-based, the only function in DeFlowd
...
- Cancellation as _scheduling state_ (`run.cancel.requested`, the ready-set effect) —
...
F5.4's _"refuse to schedule where the provider cannot express the level"_ is narrowed to **one
...
through DeFlow it is true and DeFlow enforces the level itself; when false DeFlow can enforce
...
1. `decidePermission` is a pure function in `@DeFlow/core` with no import capable of I/O; an
...
8. The four `PermissionOptionKind` values DeFlow offers back (`allow_once`, `allow_always`,
...
| 6   | contract    | Every permission response DeFlow emits validates against `schema/schema.json` with `ajv`                     | An option shape drifts from the SDK               |
| 7   | unit        | Package-boundary assertion: `@DeFlow/core`'s dependency closure contains nothing that can perform I/O        | Someone imports `node:fs` "just for a path check" |
...
improvement over F5.3 is `ToolCallLocation.path` on an inbound permission request: DeFlow rejects
...
3. A write to an absolute path outside the worktree is denied with `path-escape:absolute`, and a
   write to an absolute path _inside_ the worktree is allowed.
...
7. A denial produces an ACP-valid rejection response _and_ a `permission.denied` ledger event
...
`go`, `tsc`, `eslint`), sourced per repo from `DeFlow.config.ts`, and routes everything else to a
...
3. The syntactic second layer forces a gate for each of: `git push --force`, `git push -f`,
   ...
   `flyway migrate`.
4. `git push --force-with-lease` on a `DeFlow/` ref is **allowed**, so the safe form is not
   collateral damage.
...
| #   | Level       | Test                                                                                                                                     | Red when                                                                 |
| --- | ----------- | ---------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------ |
| 1   | unit        | Allowlist table over 20 commands from a fixture `DeFlow.config.ts`, including a binary with a path prefix (`./node_modules/.bin/vitest`) | Matching is on the raw string rather than the resolved binary            |
...
mode does this) is a useful _additional_ signal but never the only gate for a destructive operation:
...
[security model §4](../../15-security-model.md) is explicit that it _ranks above_ the human gate:
...
step. `buildChildEnv()` in `packages/proc/src/env.ts` is the only function in DeFlowd that
...
from the user's login shell at daemon start rather than DeFlowd's inherited one, `LANG`/`LC_*`/`TZ`/
...
are easy to get wrong: `SSH_AUTH_SOCK` is dropped from _agent_ environments but **kept** for
DeFlow's own git invocations, because the agent never pushes and the `Git` wrapper does; and the
...
| 3   | integration | Fake agent binary prints `process.env` as JSON; assert on what the _child_ actually received, not on what DeFlow constructed | DeFlow builds the env correctly and then merges `process.env` at spawn |
...
unit test over `buildChildEnv()` alone cannot see it. Assert on what the child _received_.
...
command line, **so that** per-node policy is possible without DeFlow ever reading or writing the
operator's own settings files.

**Decision D12**: DeFlow does not build a sandbox — it owns policy and mediation, the vendor CLI
...
string. That is what makes per-node policy compatible with AR-1 and with "DeFlow must not mutate the
...
_silently degrades_, running unsandboxed when bubblewrap and socat are missing or the platform is
...
settings are version-gated at fine granularity, and DeFlow must detect the CLI version and **degrade
...
7. DeFlow never wraps a self-sandboxing CLI in its own bubblewrap or Seatbelt profile; for CLIs with
...
`SandboxPolicy::ExternalSandbox { network_access }` would let Codex skip its own sandbox when DeFlow
...
4. Kill verification excludes `Z`-state processes:
   ...
   test asserts that the naive check _without_ the `Z` filter would have reported failure.
...
precisely: declared path scopes are **plan-time admission control** — a _prediction_, dependent on
agent compliance, whose violation is a **warning** recorded on the node — while
`merge-tree --write-tree` is **run-time gating**, _ground truth_, requiring zero agent cooperation,
whose conflict is a **gate**. The one place declared scope still bites hard is at request time:
`ToolCallLocation.path` on an inbound `session/request_permission` lets DeFlow reject an
...
through and record `provider.auth_mode = "api_key"` on the run manifest. The invariant is that _the
effective auth mode of every provider is a recorded, rendered fact, and never something the user has
to infer from a bill._ This story is P1 because F3.8 is P1 in the PRD — but its stripping half is
already implemented by KAR-08.4's allowlist, so what remains is detection and reporting.

**Acceptance criteria**

1. With `ANTHROPIC_API_KEY` set in DeFlowd's environment and a node configured for subscription
...
5. Copilot CLI's `authMethods` entry carrying an `_meta["terminal-auth"]` block is surfaced to the
   operator as a **shell command to run themselves**; DeFlow never runs it and never captures its
   output.
...
| 4   | unit        | An `authMethods` entry with `_meta["terminal-auth"]` renders as an instruction; assert no spawn occurs                              | DeFlow runs the vendor's login flow                     |
...
| **A5-2: macOS 26 Tahoe broke Seatbelt profiles in practice.** Whether Claude Code 2.1.220 fixes it is unverified.                                                                                                                                                              | **High (macOS)** | Test on Tahoe **before** promising `worktree`-level enforcement on current macOS. If it is still broken, the honest position is that macOS `worktree` enforcement rests on DeFlow's own mediation layer alone, and the UI must say so rather than implying an OS boundary that is not there. |
...
| **Prompt-injected agent reaching the daemon control plane** — an agent DeFlow spawned can reach `127.0.0.1:7777`.                                                                                                                                                              | Medium           | Not this epic: the bearer token is [EPIC-15](./EPIC-15-daemon-api.md) KAR-15.2. Flagged here because the threat model pairs the two, and neither control is sufficient alone.                                                                                                                |
