...
A fresh clone of the repository reaches a running Karvan daemon serving its UI at
...
Karvan rather than about tooling.
...
- One root `vitest.config.ts` with four project slices, and the `@karvan/testkit` fixtures they all
...
- Mechanical guard tests for the rules that are silent in development and fatal in the published
  package: no `paths` aliases, no deep imports, no `server.proxy`, `@karvan/core` has no I/O
...
- The contents of the packages. `packages/core/src/` gets an `index.ts` that exports nothing but
  ...
  EPIC-02.
- The mock agent binary and the fake exec-shim agent. `@karvan/mock-agent` and
  `@karvan/testkit/bin/fake-agent.ts` are created as empty packages here so the workspace shape is
...
`@karvan/core`'s purity is structural rather than aspirational.
...
`packages/cli`, named `karvan` — is ever published. Every `@karvan/*` package is `"private": true`
and is inlined into the CLI bundle by tsdown via `noExternal: [/^@karvan\//]`, which deletes the
...
convention: **R1** `@karvan/core` depends on nothing in the workspace and on nothing that can
perform I/O; **R2** nothing depends on `@karvan/daemon` except `packages/cli`.

`@karvan/mock-agent` deliberately does **not** depend on `@karvan/core`. If it did, a bug in the
...
6. `.node-version` and `.tool-versions` are committed so `fnm`, `nvm` and `mise` all pick the major
   up automatically.
7. `packages/core/test/purity.test.ts` asserts that `@karvan/core`'s dependency list is exactly
   `['zod']` and that no file under `packages/core/src/` (excluding tests) contains
   `from 'node:`.
8. A second guard test asserts R2: no package other than `packages/cli` lists `@karvan/daemon` as a
   dependency.
9. `@karvan/mock-agent`'s `package.json` has no workspace dependencies at all, and a guard test
   asserts it.
...
| 2 | unit | Parse every `packages/*/package.json`; assert every non-`@karvan/*` dependency value is `catalog:` and every `@karvan/*` value is `workspace:*` | A literal version appears → nine packages can now drift |
...
| 5 | unit | Assert no `package.json` except `packages/cli` depends on `@karvan/daemon` | R2 broken — the daemon has stopped being a leaf |
| 6 | unit | Assert `@karvan/mock-agent` has zero `@karvan/*` dependencies | The oracle now shares code with the thing it is meant to check |
...
3. `pnpm typecheck` runs `tsc -b` **and** `pnpm --filter @karvan/web exec vue-tsc --noEmit`, and
...
7. A guard test asserts no source file imports another workspace package by a deep path
   (`@karvan/core/src/...`) — only the package root.
...
| 5 | unit | Grep `packages/*/src/**/*.ts` for `@karvan/[a-z]+/` | A deep import appears → it breaks the `publishConfig` source→dist swap and makes an internal file public API |
...
This is D10 made concrete. `KARVAN_DEV=1 node --watch --watch-path=packages
--env-file-if-exists=.env packages/daemon/src/main.ts` starts `karvand`; Hono on
`@hono/node-server` binds the port; the API mounts on `/api` **first** so it always wins over the
SPA fallback; and when `KARVAN_DEV=1`, Vite is dynamically imported and created with
...
*process shape* and the restart behaviour; the replay-on-start path it enables is implemented in
...
8. A guard test asserts `vite` is imported only dynamically and only under the `KARVAN_DEV === '1'`
...
slices and a `@karvan/testkit` that provides the four fixtures every later epic depends on,
**so that** the test suite runs in seconds, offline, with no credentials, and tests the surfaces
where Karvan's bugs actually live rather than testing its own mocks.
...
`e2e` (a real `karvand` on an ephemeral port; 180 s timeout, `forks`, `singleFork`,
...
4. `@karvan/testkit` exports a `tmp` fixture built on `fs.mkdtemp(os.tmpdir(), 'karvan-')` that
   removes the directory on teardown **only when `KARVAN_KEEP_TMP` is unset**.
5. `@karvan/testkit` exports a `makeRepo({ branches, files, conflicts })` helper that runs real
...
6. `@karvan/testkit` exports an `agentPath` fixture that symlinks a fake binary into a tmp `bin/`
   and prepends it to `PATH`, and **also** resolves and returns the binary's absolute path — because
   karvand's `PATH` at daemon-start differs from the user's login shell, so production code stores
   the absolute path rather than relying on lookup at spawn time.
7. `@karvan/testkit` exports a `TestClock` implementing the `Clock` port (`now`, `sleep`,
...
| 3 | integration | Use the `tmp` fixture, fail the test deliberately with `KARVAN_KEEP_TMP=1`, assert the directory still exists afterwards | The directory is removed → post-mortem on a broken worktree becomes impossible, in CI especially |
...
research proxy, so versions came from the registry and unpacked tarballs while *feature and status
claims* came from search summaries (A2-4). Re-read the two official config references before
...
integration slices, with `KARVAN_KEEP_TMP=1` and `actions/upload-artifact@v4` on failure over
`/tmp/karvan-*`. `browser-e2e` runs on `ubuntu-26.04` **only** — do not triple macOS minutes on a
