...
| **`karvan` CLI** | `packages/cli`, the one published package. Bins: `karvan`, `karvan-mcp`, `karvan-mock-agent` |
| **karvand** | The local daemon the CLI starts, attaches to, or refuses to start twice. Spawned **detached**, so it outlives its launcher |
| **Repository** | A real git working copy created by `git init -b main` in an `fs.mkdtemp` directory with `GIT_CONFIG_GLOBAL=/dev/null` |
| **Global state dir** | `$XDG_DATA_HOME/karvan`, else `~/.karvan`: `karvan.lock`, `ledger.db` (+ `-wal`, `-shm`), `blobs/`, `recordings/`, `pre-migrate-<user_version>.db`, `logs/` |
| **Provider agent** | A vendor CLI. In every scenario here it is `karvan-mock-agent` symlinked onto a temp `PATH` under a vendor name, or deliberately absent |
...
| **The tarball** | `karvan-0.1.0.tgz` produced by `pnpm pack` — the exact bytes a user would get, and a different program from the workspace |
...
Background:
  ...
  And XDG_DATA_HOME points at a directory inside that same tmpdir, so no test touches ~/.karvan
  ...
  And no scenario calls vi.useFakeTimers() while karvand or an agent child process is alive
  ...
  And karvand binds an ephemeral port in tests and 127.0.0.1:7777 in the documented flows
...
> verified false-negative trap: after a *successful* group SIGKILL, `ps` still lists the
...
|---|---|---|---|
| EPIC-18-S1 | Happy path: `karvan init` bootstraps a repository | KAR-18.1 | Happy path |
...
| EPIC-18-S7 | Happy path: `karvan up` to a browser in under three seconds | KAR-18.2 | Happy path |
| EPIC-18-S8 | **A second `karvan up` is refused by the single-instance lease** | KAR-18.2 | Concurrency |
...
| EPIC-18-S18 | Happy path: `karvan run` with no daemon running | KAR-18.3 | Happy path |
| EPIC-18-S19 | `karvan run` attaches to a daemon that is already up | KAR-18.3 | Happy path |
...
| EPIC-18-S38 | `@karvan/*` inlined, `@lydell/node-pty` external | KAR-18.5 | Edge case |
...
| EPIC-18-S47 | `karvan ledger snapshot` produces one consistent, sidecar-free file | KAR-18.7 | Happy path |
| EPIC-18-S48 | `karvan status` with a live daemon, and with none at all | KAR-18.7 | Happy path |
| EPIC-18-S49 | `karvan status` after a SIGKILL reports `stale`, and signals nothing | KAR-18.7 | Recovery |

---

## EPIC-18-S1 — Happy path: `karvan init` bootstraps a repository
...
Feature: workspace bootstrap
  ...
  Scenario: init in a clean repository
    Given a git repository on branch "main" with no ".karvan" directory
    And a ".gitignore" containing only "node_modules/"
    When the operator runs "karvan init"
    Then ".karvan/config.yaml" exists and validates against ".karvan/schemas/config.schema.json"
    And ".karvan/gates/", ".karvan/templates/" and ".karvan/memory/" exist
    And ".karvan/.worktreeinclude" exists
    And ".gitignore" now also contains ".karvan/daemon.json", ".karvan/wt/" and ".karvan/runs/"
    And the global state directory is created and its absolute path is printed
    And stdout lists each path with "created"
    And the command exits 0
    And "git status --porcelain" shows only ".karvan/" and ".gitignore" as changes
```

**Notes:** the two halves of `.karvan/` are not interchangeable — the committed half is a team
...
Feature: workspace bootstrap
  ...
  Scenario: init is idempotent and non-destructive
    Given "karvan init" has already run
    And the operator has edited ".karvan/config.yaml" to set a run budget ceiling of 25.00
    And the operator has added ".karvan/gates/typecheck.yaml"
    When the operator runs "karvan init" a second time
    Then ".karvan/config.yaml" still contains the budget ceiling of 25.00
    And ".karvan/gates/typecheck.yaml" is untouched
...
Feature: workspace bootstrap
  ...
  Scenario: not a repository
    ...
    Then stderr contains
        "karvan init: not inside a git working tree (run 'git init' first)"
    And the exit code is 5
    And no ".karvan" directory was created
    And no ".gitignore" was created
```

**Notes:** every downstream mechanism — worktrees, flat branch names `karvan/<runId>__<nodeId>`,
...
Feature: workspace bootstrap
  ...
  Scenario Outline: gitignore merge is a fixpoint
    ...
    When the operator runs "karvan init" twice
    Then ".karvan/daemon.json" appears exactly once in ".gitignore"
    And ".karvan/wt/" appears exactly once
    And ".karvan/runs/" appears exactly once
    And "git check-ignore -q .karvan/daemon.json" exits 0
    ...
    Examples:
      ...
      | already contains ".karvan/wt/" and nothing else    |
...
Feature: workspace bootstrap
  ...
  Scenario: detection result is machine-local
    ...
    And ".karvan/config.yaml" contains no capability field for any provider
    And ".karvan/config.yaml" contains no absolute path
```
...
## EPIC-18-S7 — Happy path: `karvan up` to a browser in under three seconds
...
Feature: daemon lifecycle
  ...
  Scenario: first boot
    ...
    When the operator runs "karvan up --timings --no-open"
    Then the daemon binds 127.0.0.1 and no other interface
    And ".karvan/daemon.json" exists with { pid, port, token, startedAt } at mode 0600
...
## EPIC-18-S8 — A second `karvan up` is refused by the single-instance lease
...
Feature: daemon lifecycle
  ...
  Scenario: two terminals, one daemon
    Given karvand is running with pid 4711 on port 7777 holding the flock on
          "<dataDir>/karvan.lock" with daemon_epoch = 3
    When the operator runs "karvan up" in a second terminal against the same data directory
    Then stderr contains
        "karvan up: another karvand is already running (pid 4711, port 7777) — open
         http://127.0.0.1:7777 or run 'karvan status'"
    And the exit code is 2
    And daemon_epoch is still 3
    And ".karvan/daemon.json" still holds pid 4711
    And no second HTTP listener was bound
    And no event was appended to the ledger by the second process
```

**Notes:** this is described in the architecture as *"very common"*, and it is the reason the lease
exists at all. SQLite enforces one *writer*, but that does not stop two schedulers interleaving
...
Feature: daemon lifecycle
  ...
  Scenario: port collision with an unrelated process
    ...
    And no karvand holds the lease
    When the operator runs "karvan up --no-open"
    Then the daemon binds the next free port reported by get-port
    And the printed URL carries that port
    And ".karvan/daemon.json".port equals that port
    And "karvan status" reports that port
...
authenticate. Note this is *not* the two-daemon case: 7777 being busy is not evidence that Karvan is
...
migrations, so the backup *is* the rollback story.
...
Feature: daemon lifecycle
  ...
  Scenario: boot after kill -9
    Given karvand was running with a mock agent child mid-turn
    And the daemon was killed with SIGKILL, leaving ".karvan/daemon.json" behind
    ...
    And ".karvan/daemon.json" is rewritten with the new pid, port and token
...
Feature: daemon lifecycle
  ...
  Scenario: a recycled pid must not be killed
    ...
    When karvand boots and runs orphan reaping
...
Feature: graceful degradation
  ...
  Scenario: a fresh laptop with nothing but node, pnpm, git and a browser
    ...
    And stdout states that "karvan replay" and the bundled mock agent work with no provider
  ...
  Scenario: what the daemon refuses, and what it still allows
    ...
    When the operator submits a task through "karvan run"
    ...
    And "karvan replay fixtures/happy-path-12.jsonl" serves a full run over the same HTTP contract
```
...
degrades the plan rather than killing the run; *zero* providers is the same rule taken to its limit.
The second scenario matters because it is what makes the whole UI developable on a train: the mock
agent binary and `karvan replay` ship inside the tarball.
...
Feature: daemon lifecycle
  ...
  Scenario: SIGINT with work in flight
    Given karvand is running with one mock agent child mid-turn
    ...
    And the flock on "<dataDir>/karvan.lock" is released
    And ".karvan/daemon.json" no longer exists
    And the interrupted node is recorded in the ledger with a typed failure, not left "running"
    And a subsequent "karvan up" starts cleanly with no manual intervention
```
...
## EPIC-18-S18 — Happy path: `karvan run` with no daemon running
...
Feature: headless execution
  ...
  Scenario: one command, one completed run, no browser
    ...
    When the operator runs: karvan run "add a health endpoint"
    Then karvand is spawned detached and "GET /api/health" is polled until it answers
    ...
    And the gate verdict and the branch name "karvan/<runId>__<nodeId>" are printed
...
**Notes:** this is the story the roadmap gates the view work on: *"Do not start W11 until at least
one full run completes headlessly."* It is also the recorder — the six replay fixtures are recorded
...
## EPIC-18-S19 — `karvan run` attaches to a daemon that is already up
...
Feature: headless execution
  ...
  Scenario: attach, never launch a second
    Given karvand is already running and ".karvan/daemon.json" is current
    When the operator runs: karvan run "…"
    Then no second karvand process is spawned
    And daemon_epoch is unchanged
    And the CLI authenticates with the bearer token read from ".karvan/daemon.json"
...
Feature: headless execution
  ...
  Scenario: detach
    Given "karvan run" is streaming a run with a mock agent mid-turn
    When the operator presses Ctrl-C once
    Then stdout contains
        "detached — run <runId> continues; 'karvan run --attach <runId>' to watch,
         'karvan cancel <runId>' to stop"
...
Feature: headless execution
  ...
  Scenario: SIGKILL the launcher
    Given "karvan run" autostarted karvand and is streaming a run
    When the CLI process is killed with SIGKILL
    Then karvand is still alive
    And karvand is not a member of the CLI's process group
    And the agent child process is still alive
    And the run reaches a terminal state
    And "karvan run --attach <runId>" from a new terminal shows the completed transcript
```

**Notes:** the daemon owns execution — *"close the browser, close the laptop lid, reboot, the run
resumes"* — and that property starts with the daemon not being a child of whatever launched it. If
`karvand` were spawned in the CLI's process group, closing the terminal would take down an
...
Feature: headless execution
  ...
  Scenario: resume by explicit cursor
    Given "karvan run" has rendered events up to seq 41
...
Feature: headless execution
  ...
  Scenario Outline: the exit code is a closed contract
    ...
    When "karvan run" observes it with flags "<flags>"
...
Feature: headless execution
  ...
  Scenario Outline: F1.1 sources
    ...
    When the operator runs "karvan run <argument>"
    ...
    And ".karvan/runs/<runId>/" records the source on disk in an open format
    ...
    Examples:
      ...
      | --spec .karvan/specs/x.yaml  | spec  | .karvan/specs/x.yaml |
```

**Notes:** recording the *locator* as well as the content is what lets the node inspector answer
...
Feature: headless execution
  ...
  Scenario: machine-readable output
    ...
    When the operator runs "karvan run --json '…'"
...
Feature: environment probe
  ...
  Scenario: everything is fine, and it says so specifically
    ...
    And a writable global state dir and a writable repo-local ".karvan/"
...
**Notes:** the absolute resolved path is not decoration. karvand's `PATH` at daemon start differs
...
Feature: environment probe
  ...
  Scenario: nothing installed
    ...
    And the report states that the bundled mock agent and "karvan replay" need no provider
    And the exit code is 0
    And stderr is empty
```
...
prediction precisely because agents violate them, so this is the gate. Below 2.38 Karvan refuses to
...
Feature: environment probe
  ...
  Scenario: the fix is named, and the fail-closed policy is stated
    ...
    And the report states that Karvan sets "sandbox.failIfUnavailable: true" and
...
in the system notices. Reporting *honourable levels* rather than "sandbox: ok" is what makes the
...
Feature: environment probe
  ...
  Scenario: AR-1 holds even while reporting on auth
    When the operator runs "karvan doctor" under a syscall or fs-access assertion
...
**Notes:** the failure mode this exists for is stated in the PRD in one sentence: *"you thought you
were on your subscription and you were being billed."* Nothing else in the system can detect it,
...
Feature: environment probe
  ...
  Scenario Outline: a mock agent wearing each vendor's capability profile
    Given "karvan-mock-agent --agent-capabilities <profile>" is on the temp PATH as "<name>"
    When the operator runs "karvan doctor"
    Then the regenerated matrix records session.resume = <resume>
    And it records fork = <fork> and list = <list>
    And the report states the resume strategy Karvan would use for that adapter
...
across all five agents probed — so a pty is needed only for Karvan's own ACP `terminal/*`
...
is also the only native install risk left in `npx karvan up`.
...
Feature: environment probe
  ...
  Scenario Outline: one status model, two renderings
    ...
    When "karvan doctor" and "karvan doctor --json" are both run against it
...
## EPIC-18-S38 — `@karvan/*` inlined, `@lydell/node-pty` external
...
Feature: packaging
  ...
  Scenario: exactly one native runtime dependency survives
    ...
    Then no import specifier begins with "@karvan/"
    ...
    And "packages/cli/package.json" lists no "@karvan/*" dependency
```

**Notes:** `noExternal: [/^@karvan\//]` is what makes the single-package design work — every
`@karvan/*` package is private, so a leaked specifier means npm tries to fetch a name that does not
exist and the install fails for the user with a 404. `vite` is a devDependency imported dynamically
only under `KARVAN_DEV === '1'`; if it appears in the bundle, the published package drags a dev
server behind it.
...
Feature: packaging
  ...
  Scenario: only the package root is importable
    Given a source file importing "@karvan/core/src/reduce.ts"
...
Feature: install verification
  ...
  Scenario: the exact bytes a user would get
    ...
    And "pnpm pack" has produced "karvan-0.1.0.tgz"
    ...
    When the following runs in that directory:
      ...
      | cd demo                               |
      | npx /path/to/karvan-0.1.0.tgz init    |
      | npx /path/to/karvan-0.1.0.tgz up      |
    ...
    And an asset referenced by that HTML under "/assets/" returns 200 with a JavaScript
        content type
    And with the tarball's own "karvan-mock-agent" on PATH, a scripted multi-node run driven by
        "npx /path/to/karvan-0.1.0.tgz run" completes and exits 0
    And no node-gyp process was spawned during the install
    And the clean directory is removed on success, and preserved under KARVAN_KEEP_TMP=1
```
...
**Notes:** this is the named classic — *"a missing `files` entry that drops `dist/ui/`, so the daemon
starts and serves a blank page"* — and this scenario exists to prove the verifier catches it rather
...
## EPIC-18-S47 — `karvan ledger snapshot` produces one consistent, sidecar-free file
...
Feature: diagnostics
  ...
  Scenario: attach my ledger to this bug report
    ...
    When the operator runs "karvan ledger snapshot <runId> --out /tmp/karvan-bug-1234.db"
    ...
    And
      ...
      returns rows when run with the plain sqlite3 CLI and Karvan not installed
...
## EPIC-18-S48 — `karvan status` with a live daemon, and with none at all
...
Feature: diagnostics
  ...
  Scenario: a live daemon
    Given karvand is running with two active runs
    When the operator runs "karvan status"
    ...
    And "karvan status --json" reports the same values in one JSON document
    And the exit code is 0

  Scenario: no daemon at all
    Given no ".karvan/daemon.json" exists
    When the operator runs "karvan status"
...
## EPIC-18-S49 — `karvan status` after a SIGKILL reports `stale`, and signals nothing
...
Feature: diagnostics
  ...
  Scenario: the daemon.json outlived its daemon
    Given karvand was killed with SIGKILL, leaving ".karvan/daemon.json" behind
    When the operator runs "karvan status"
    Then stdout reports the daemon as "stale" and names ".karvan/daemon.json"
    And it states that "karvan up" will take over cleanly
    And no signal is sent to the recorded pid
    And the exit code is 0

  Scenario: the recorded pid has been recycled
    Given ".karvan/daemon.json" records a pid now held by an unrelated process
...
looking for a process that is not Karvan, and — worse — invites them to kill it.
