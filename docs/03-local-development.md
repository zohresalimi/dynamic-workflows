...
- **One process, one port.** `pnpm dev` starts `karvand` and nothing else. Vite runs *inside* it
  (D10).
...
- **`karvan replay`**, which serves the normal API from a recorded run, so all nine P0 views are
...
Nothing else is required to build, run and test Karvan.
...
`karvan doctor` checks both and refuses to schedule write nodes below 2.38.

### Optional, per platform

These are only needed once you point Karvan at a **real** agent CLI. Everything in this document up
...
`karvan doctor` check, not a footnote: Claude Code's sandbox **silently falls back to running
...
```bash
git clone https://github.com/<you>/karvan.git
cd karvan
pnpm install
pnpm dev
```
...
|---|---|
| `pnpm install` | Resolves from the committed lockfile. Links the seven `@karvan/*` workspace packages by symlink. Downloads two prebuilt native binaries (`better-sqlite3`, `@lydell/node-pty`) — **no compilation**. Runs `prepare`, which is `lefthook install` (§12) |
| `pnpm dev` | Starts one Node process: `karvand` with `KARVAN_DEV=1`, which mounts Vite in middleware mode on its own HTTP server, serves the UI and the API from the same origin, and restarts on every file save |
...
// package.json (root)
{
  ...
  "scripts": {
    ...
    "dev:replay":   "KARVAN_DEV=1 node --watch --watch-path=packages packages/cli/src/bin.ts replay fixtures/happy-path-12.jsonl --speed 20x",
    ...
    "test:record":  "KARVAN_RECORD=1 vitest run --project integration --testNamePattern @live",

    "lint":         "oxlint --type-aware && biome check .",
    "format":       "biome check --write .",

    "doctor":       "node packages/cli/src/bin.ts doctor",
    "pack:check":   "pnpm build && pnpm --filter karvan exec publint && pnpm --filter karvan exec attw --pack",
...
Note what is *not* there: no `dev:web`, no `dev:api`, no `concurrently`, no `wait-on`. There is one
dev process.
...
bundled type declarations, where `server` is documented as *"Parent server instance to attach to.
This is needed to proxy WebSocket connections to the parent server"* — i.e. Vite's HMR websocket
rides `karvand`'s own HTTP server rather than opening a second one.
...
Because Karvan's entire UI is an SSE projection of the ledger (F4.1, NF10), and **Vite's dev proxy
...
|---|---|---|
| **Buffering** | Events arrive in one burst when the stream *ends*, not as they happen | The live plan graph (F10.1) and live agent streams (F10.6) would appear frozen for hours and then flood. You would spend an afternoon assuming your reducer was broken |
...
later puts a reverse proxy in front of `karvand` — but as a *dev loop* the tradeoff is absurd. You
...
For most projects that is a mild nuisance. For Karvan it is **free, continuous, adversarial testing
...
2. The new process reopens the ledger (`$XDG_DATA_HOME/karvan/ledger.db` — one global database, see
...
Karvan orchestrates other people's agents. If development required those agents, the inner loop
...
`@karvan/mock-agent` is a **shipped package with a real bin** (`karvan-mock-agent`), not a test
helper (D17). It implements the *agent* side of `@agentclientprotocol/sdk@1.3.0` and is driven by a
...
export KARVAN_MOCK_SCRIPT="$PWD/fixtures/scripts/happy-path.json"
...
| 4 | Calls back into the client: `fs/read_text_file`, `fs/write_text_file`, `terminal/create`, `terminal/output`, `terminal/wait_for_exit`, `terminal/kill`, `terminal/release` | Karvan's mediation boundary (D12) |
...
```bash
karvan-mock-agent --replay recordings/claude-code@2.1.220/permission-refusal.ndjson
```

Because ACP is NDJSON, a recording is just the byte stream plus direction. The recorder is a tee at
the transport level (`KARVAN_RECORD=1`), keyed on the exact agent version so a vendor bump produces
...
### 6.2 `karvan replay` — develop all nine views offline

```bash
karvan replay fixtures/happy-path-12.jsonl --speed 20x --port 7777
```
...
- **It is also the demo tool.** The PRD's strongest internal demo (§15.4) is a real task shown
  through the plan-evolution scrubber. That is `karvan replay` pointed at a recorded real run.
...
pnpm test:e2e          # a real karvand on an ephemeral port, driven end to end
```
...
| `e2e` | `e2e/**/*.test.ts` | Boot `karvand`, run a mock plan to completion, scrub plan versions in a browser | `singleFork`, `fileParallelism: false`, 180 s timeout |
...
Snapshots need a normalising serializer registered *before* the first one is written, or every
...
| Category | Checks |
|---|---|
| **Runtime** | Node >= 24; pnpm version; writable global state dir (`$XDG_DATA_HOME/karvan` or `~/.karvan`); writable repo-local `.karvan/` |
...
a vendor CLI reports a login command, Karvan prints it for the user to run themselves.

---

## 9. `npx karvan up` — first run, step by step
...
1. **Resolve directories.** Global state at `$XDG_DATA_HOME/karvan` (or `~/.karvan`); per-repo state
   at `.karvan/` in the current repo, per PRD §9.4. The split matters: `.karvan/` can be
...
3. **Take the lock.** `flock` on `~/.karvan/karvan.lock`, bump `daemon_epoch`. This is what stops a
   double-launched daemon — very common, since people run `npx karvan up` in two terminals — from
   driving the same run twice.
4. **Probe providers.** The `doctor` path (§8): which binaries exist, their versions, their
   capabilities. Karvan plans only against what is actually available (PRD §5.3, F5.4).
5. **Pick a port.** 7777, or the next free one via `get-port`.
6. **Generate a token.** 32 random bytes. Write `.karvan/daemon.json` as `{ pid, port, token,
   startedAt }`.
7. **Bind `127.0.0.1` only — and still authenticate.** A localhost bind is *not* a security
...
cd packages/cli && pnpm pack          # -> karvan-0.1.0.tgz
cd "$(mktemp -d)"
git init -b main demo && cd demo
npx /path/to/karvan-0.1.0.tgz up      # the exact bytes a user would get
```
...
- A `@karvan/*` package that failed to inline and is now an unresolvable runtime import.
...
The build order is fixed and matters: `pnpm --filter @karvan/web build` → `packages/web/dist` →
copied into `packages/cli/dist/ui/` → `tsdown` bundles `bin.ts` with `@karvan/*` inlined and the two
natives external.
...
```bash
KARVAN_KEEP_TMP=1 pnpm test:int
```

The tmpdir fixture only removes the directory when `KARVAN_KEEP_TMP` is unset. With a failed
...
```sql
VACUUM INTO '/tmp/karvan-bug-1234.db';
```

Or from the CLI:

```bash
karvan ledger snapshot <runId> --out /tmp/karvan-bug-1234.db
```
...
```bash
sqlite3 /tmp/karvan-bug-1234.db \
  "SELECT seq, kind, node_id, attempt FROM event WHERE run_id='<id>' ORDER BY seq LIMIT 50;"
sqlite3 /tmp/karvan-bug-1234.db "PRAGMA integrity_check;"
```
...
```
.karvan/runs/<runId>/       exports, written from the ledger — not the source of truth
...
.karvan/wt/<runId>__<nodeId>/   git worktrees, one per write-capable node (D13)
$XDG_DATA_HOME/karvan/ledger.db  the event log — the source of truth, one global database
```
...
karvan replay fixtures/gate-fail-repair.jsonl --speed 50x   # jump straight to a failed gate
karvan replay fixtures/stress-400.jsonl --speed max         # graph performance
```
...
| Port 7777 is in use / two daemons | A second `karvan up` in another terminal | The `flock` + `daemon_epoch` fence rejects stale-epoch writes. Check `.karvan/daemon.json` for the live pid |
...
| Claude Code appears sandboxed but writes outside the worktree | `bwrap`/`socat` missing, or AppArmor blocking user namespaces — the sandbox falls back to unsandboxed **silently** | `pnpm doctor`; install bubblewrap and socat; add the `/etc/apparmor.d/bwrap` profile. Karvan sets `sandbox.failIfUnavailable: true` for every non-`full` level |
| Agent CLIs keep running after `karvand` is SIGKILLed | Children reparented to init | They are spawned `detached: true` so the whole process group can be killed. On restart, orphans are reaped by matching `(pid, process_start_time)` — never by bare pid |
