...
This document specifies the Workspace Manager and the safety model: how Karvan drives git, how it
...
It implements PRD §7.5 (F5.1–F5.8) and is the mechanical backing for the risk row *"destructive
action at the execution boundary"*.
...
> **Decision (locked).** Karvan invokes the system `git` binary through `execa@^10.0.1`, wrapped in
...
| `nodegit` | 0.27.0 stable / 0.28.0-alpha.38 (2026-04-23) | Needs `node-gyp` native compilation, declares `engines.node >= 6`, and has been in alpha for three years. Breaks `npx karvan up` (NF6) on every Node ABI bump. It is libgit2-based, and **libgit2 still does not support relative worktrees** (libgit2#7210). |
...
Karvan is already a program whose job is spawning other people's binaries under the user's account.
...
export class Git {
  ...
  async run(args: readonly string[], opts: { cwd?: string; timeoutMs?: number } = {}): Promise<GitResult> {
...
Note `reject: false`. `git merge-tree` uses exit code 1 to mean *conflict*, which is a normal,
...
### 1.2 Minimum git version, enforced in `karvan doctor`

| Requirement | Version | Why |
|---|---|---|
| Hard floor | **git >= 2.38** | `git merge-tree --write-tree` — the entire conflict-detection design (§6) does not exist below this. |
| Preferred | **git >= 2.45** | Stable `--porcelain -z` on `worktree list`, which is how Karvan reads worktree state (§4.2). |

Below 2.38, `karvan doctor` fails hard and the daemon refuses to start a run. Between 2.38 and 2.45
...
`extensions.relativeWorktrees`, which is incompatible with older git *and* unsupported by libgit2
...
> **Decision D13 (locked).** Branch names are flat: `karvan/<runId>__<nodeId>`.
> Integration branches live under a reserved, non-colliding prefix: `karvan/int/<runId>`.

The PRD's F5.1 specifies `karvan/<run-id>/<node-id>`. That scheme forecloses a run-level integration
branch, which §7 requires.

**Verified 2026-08-02, git 2.43.** After `karvan/r1/n1` exists:
...
`refs/heads/karvan/r1` cannot be both a file and a directory. The hierarchical scheme works only as
...
(`karvan/run-1/node.lock` → INVALID), `..` sequences, trailing spaces, and `@{`.
...
export function nodeBranch(runId: string, nodeId: string): string {
  if (!BRANCH_SAFE.test(runId) || !BRANCH_SAFE.test(nodeId)) throw new UnsafeRefError(runId, nodeId);
  return `karvan/${runId}__${nodeId}`;               // flat — D13
}
export const integrationBranch = (runId: string) => `karvan/int/${runId}`;
```

**Alternative not taken:** a custom ref namespace, `refs/karvan/<run>/<node>`, outside
...
Not *"is already checked out at"*, which is what most blog posts and most people's notes claim.
...
real-world hit, and it happens on the *very first run*, on the branch the user is sitting on. The
error message must say so in Karvan's own words, not git's.
...
> **Karvan must never pass `--force` to `worktree add`.** `--force` is permitted only on
> `worktree remove`.
...
function assertNoForcedWorktreeAdd(args: readonly string[]): void {
  if (args[0] === 'worktree' && args[1] === 'add' && (args.includes('--force') || args.includes('-f')))
...
```bash
# write node
git -C <mainRepo> worktree add --lock --reason "karvan run=<runId> node=<nodeId>" \
    -b karvan/<runId>__<nodeId> <path> <baseRef>

# read-only node
git -C <mainRepo> worktree add --detach --lock --reason "karvan run=<runId> node=<nodeId>" \
    <path> <baseRef>
```

- **`--lock` is applied atomically at creation.** Do not create-then-lock — that races with Karvan's
...
gives exactly-once semantics against Karvan's own reaper, across daemon restarts, without inventing
...
releases locks whose owning process has exited. Karvan copies that, **including the stale-lock
release** — see §11.3.
...
| Situation | git says | Karvan does |
|---|---|---|
...
| Worktree contains only gitignored files (e.g. `node_modules/`) | *nothing* — `remove` succeeds, exit 0, no `--force` needed | nothing special |

**The dirty-removal path.** Karvan must never blind-force, because forcing discards work an agent
...
2. Auto-commit everything to the node branch as a `karvan: WIP salvage` commit, so the work is
   recoverable by ref.
3. *Then* `git worktree remove --force <path>`.
...
removed cleanly with exit 0. So a fat `node_modules` is a *disk* problem (§5), not a removal
...
Every worktree tool creates worktrees; almost none of them make the worktree *work*. The verified
2026 landscape treats dependency setup as the user's problem — Claude Code's own docs concede it:
*"A worktree is a fresh checkout, so initialize your development environment there: ask Claude to
install dependencies."* Standalone tools exist purely to symlink and run setup scripts.

Solving this properly is cheap and immediately felt. It is a genuine Karvan differentiator.

### 5.1 Three layers

**Layer 1 — copy gitignored config (always on, cheap).** Adopt Claude Code's `.worktreeinclude`
convention *verbatim*: gitignore syntax, and only files that match a pattern **and** are gitignored
...
**pnpm is the recommended default** and the thing Karvan should nudge users toward, because its
...
`pnpm.io/git-worktrees` could not be fetched (403) — the store-sharing *mechanism* is long-standing
...
> to provide. The one safe symlink target is the *store*, which pnpm already does correctly.

**Layer 3 — explicit escape hatch.** `karvan.config.ts` exposes `workspace.setup`, a command run once
...
`clonefile` on macOS) is genuinely near-free and instant *where supported*. Add it behind a
...
A worktree costs one full checkout of *tracked* files. **The object store is shared** — verified: a
...
**The pipe-exit-code trap.** The exit code *is* the conflict signal. If you pipe `merge-tree`
...
```ts
export async function mergeTree(git: Git, a: string, b: string): Promise<ConflictProbe> {
  const r = await git.run(['merge-tree', '--write-tree', '--name-only', '-z', a, b]);
  if (r.exitCode === 0) return { clean: true, paths: [] };
  if (r.exitCode === 1) return { clean: false, paths: r.stdout.split('\0').slice(1).filter(Boolean) };
...
After each write node commits, Karvan runs `merge-tree` of that node's branch against:

- (a) the run's integration branch `karvan/int/<runId>`, and
...
serialize two nodes the moment their branches *start* conflicting, rather than discovering it after
...
|---|---|---|---|
| **Declared path scopes** (F5.3) | Plan-time **admission control** — do not schedule two nodes whose declared scopes obviously collide | A *prediction*, dependent on agent compliance | A violation is a **warning** recorded on the node |
| **`merge-tree --write-tree`** (D14) | Run-time **gating** — this is what actually happens on merge | *Ground truth*, requires zero agent cooperation | A conflict is a **gate** |
...
inbound permission request lets Karvan enforce path scope **at request time** rather than detecting
...
F5.2's *"serialize writes, parallelize reads"* is confirmed as the industry consensus. Integration
guidance is equally consistent: merge sequentially, lowest-overlap-first, verify after each — never
a big-bang N-way merge.

### 7.1 The loop

1. Create `karvan/int/<runId>` from the run's base ref at run start (which is why §2 exists).
...
Karvan spawns a resolution node at `worktree` permission, on the integration worktree, whose context
...
### 7.3 The refinement over the PRD: serialize on *detected*, not *declared*, overlap
...
> **Decision (locked).** The ladder (F5.4) is enforced by Karvan at the ACP client boundary, not by
...
(`PROTOCOL_VERSION = 1`): the `CLIENT_METHODS` that Karvan implements include
...
**Karvan sits in the path of every file access and every command execution, for every vendor.** The
...
| Type | Verified shape | Use in Karvan |
|---|---|---|
| `PermissionOptionKind` | `"allow_once" \| "allow_always" \| "reject_once" \| "reject_always"` | The options Karvan offers back on `session/request_permission` |
...
`ToolCallLocation.path` is the improvement over F5.3 noted in §6.3: Karvan rejects the write *before
it happens*, with a reason the UI can render, rather than diffing at completion and calling it a
gate failure. **Karvan auto-responds** to routine `session/request_permission` calls from the policy
...
reintroduced in the safety layer. Under ACP it is near-moot: Karvan enforces the level itself
regardless of what the vendor can express.

> **Reduce F5.4's refusal rule to one capability bit: `mediatedExecution: true | false`.**

It is true when the adapter routes `fs/*` and `terminal/*` through Karvan, false otherwise. When
false, Karvan cannot enforce anything and would be trusting vendor flags — so it refuses to schedule
...
permission prompts to an MCP tool Karvan hosts — the same interception pattern, one vendor at a
...
> **Decision D12 (locked).** Karvan does not build a sandbox. It owns policy and mediation; the
> vendor CLI owns enforcement.

### 9.1 Layer 1 — Karvan: worktree boundary + process containment (always on, free)

The worktree *is* the primary filesystem boundary. Every agent is spawned with `cwd` set to its
...
> **The crucial integration fact: the CLI accepts `--settings '<inline JSON>'`.** Karvan can inject
...
> with "Karvan must not mutate the user's vendor CLI configuration".
...
> **For any non-`full` level, Karvan must set `failIfUnavailable: true` and
...
built-in credential deny list — Karvan must populate `sandbox.credentials.files` explicitly.
...
| Karvan level | Claude Code | Codex |
|---|---|---|
...
doc comment reads *"Indicates the process is already in an external sandbox"*. If it is
user-selectable it would let Codex skip its own sandbox when Karvan provides isolation, avoiding
nesting entirely. How to select it from the CLI or `config.toml` could not be verified. Investigate
before relying on it.

### 9.3 Layer 3 — Karvan's mediated execution (the real control point)

Every command arrives at Karvan's `terminal/create` handler *before* it runs, carrying `command`,
`args`, `cwd`, `env`. That is where the allowlist and the human gates live (§10). This layer is the
one Karvan can actually test, version and reason about.
...
needs `enableWeakerNestedSandbox`, which Anthropic warns *considerably weakens security*. Wrapping a
sandboxing CLI in a Karvan-authored bwrap or Seatbelt profile turns a working sandbox into a broken
one. Karvan's leverage is policy and mediation, not enforcement primitives.
...
(*"Correcting the Financial Times report about AWS, Kiro, and AI"*, aboutamazon.com) attributing the
...
The PRD (§4.5, F5.6) says approved specs did not prevent it *"because nothing reviewed the moment of
action."* True, but incomplete — and the incomplete half is the actionable half.
...
access controls" is a control Karvan can actually implement, whereas "the AI decided badly" is not.

### 10.2 Consequence 1: scrub the child environment — this ranks *above* the human gate

The agent must not inherit credentials it does not need. Karvan builds the child environment from an
...
`go`, `tsc`, `eslint` — sourced per repo from `karvan.config.ts`, and route **everything else** to a
human gate.
...
useful *additional* signal but never the only gate for a destructive operation: it is probabilistic
...
*"Never write to the default branch"* is enforced in the `Git` wrapper (§1.1), not in a policy
document:
...
grandchildren is karvand's own, so group-signalling it would kill the daemon.
...
**Verified.** After a *successful* group SIGKILL, `ps` still lists the grandchildren. The first test
...
`detached: true` means an agent survives karvand's death, which is the price of group-kill. So:
...
| `forceKillAfterDelay` | Is **not** the tree-kill option, and "does not work when the subprocess is terminated by calling `subprocess.kill()` with a specific signal", or by `process.kill(subprocess.pid)`. Since Karvan always passes an explicit signal, there is **no automatic escalation** — the timer in §11.1 must be implemented by hand. |
...
repos already have, so Karvan reuses the user's own `.devcontainer/devcontainer.json` rather than
...
| Dagger `container-use` | No, for Karvan specifically. Good design (per-agent container + `container-use/<env>` branch), but it is an **MCP server the agent drives**, so the *agent* chooses its own isolation. Architecturally backwards for an orchestrator that must impose isolation, and it adds a Dagger engine dependency. Worth studying. |
...
For the narrow case of *"wrap a CLI that has no sandbox of its own"*,
...
| Use `karvan/<runId>/<nodeId>` | Refs directory/file conflict forecloses `karvan/<runId>` as an integration branch. **Verified.** |
| Trust `check-ref-format` alone | `-n` is a *valid* ref name that git then parses as a flag. Use `--` separators and reject leading dashes. |
...
| Symlink a shared `node_modules` across worktrees | Concurrent installs corrupt it and it defeats the isolation the worktree exists for. Share the *store*, not the tree. |
...
### 13.1 Environment checks that belong in `karvan doctor`
...
- Vendor CLI versions, and the fine-grained version gating of Claude Code sandbox settings
  ...
  `strictAllowlist` >= 2.1.219). Karvan must detect the CLI version and **degrade the ladder
