...
| `run.kill_failed`                           | `{ survivors: { node: NodeId; attempt: number; pid: number; pgid: number; stat: string }[] }` | F5.7 — a kill that did not take is an **event**, not a silent condition. `stat` is never `Z`: a zombie is already dead |
