...
| `run.needs_human`                           | `{ reason: 'churn' \| 'budget' \| 'reconcile-unknown' \| 'spec-revalidation'; detail }`  | circuit breaker trip. `spec-revalidation` is a mid-run spec edit the current plan no longer satisfies (F1.3, KAR-10.3) |
