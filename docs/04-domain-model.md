...
| ------------------------------------------- | ---------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| `task.submitted`                            | `{ sha256: string; raw?: string; handle?: Handle; provenance }`                          | F1.1 — intake's one event (KAR-10.1). `raw` xor `handle`: a source over 64 KiB goes to the blob store rather than bloating every ledger read. No interpretation — the payload carries the exact source and nothing derived from it |
| `run.created`                               | `{ spec: TaskSpec; cwd: string; repo: { head: string; branch: string } }`                | F1.1 — appended by the **framing interview** (KAR-10.2), not by intake: the payload is the `TaskSpec`, so the event cannot exist before there is one |
| `run.spec.approved`                         | `{ specHash: string; by: 'ui' \| 'cli' }`                                                | F1.3 — the gate, recorded                                                                            |
| `spec.amended`                              | `{ from: string; to: string; patch: JsonPatchOperation[]; document: TaskSpecDraft; by }`  | F1.3 — an operator edit as an RFC 6902 patch (KAR-10.3). Nothing is overwritten: the pre-edit spec is still `run.created.spec`, still addressable at `from` |
| `spec.pinned`                               | `{ specHash: string; segments: { id; kind; sha256 }[] }`                                 | F1.5 — approval mints the pinned set, in the **same transaction** as the approval it derives from     |
...
| `run.cancel.unanswered`                     | `{ mode: 'cooperative'; requestedTs: number; waitedMs: number; live: { node: NodeId; attempt: number; pid: number; pgid: number }[] }` | KAR-27.6 — a cooperative cancel that has gone unanswered past `COOPERATIVE_CANCEL_UNANSWERED_MS`, and what is still running under it. A **report**, never a promotion: cooperative is never escalated (EPIC-19-S38) |
...
| `run.kill_failed`                           | `{ survivors: { node: NodeId; attempt: number; pid: number; pgid: number; stat: string }[] }` | F5.7 — a kill that did not take is an **event**, not a silent condition. `stat` is never `Z`: a zombie is already dead. Written by the operator's kill switch and by a daemon shutdown that could not take its children with it (KAR-27.10) |
| `run.needs_human`                           | `{ reason: 'churn' \| 'budget' \| 'reconcile-unknown' \| 'spec-revalidation' \| 'plan-invalid' \| 'patch-rejected'; detail }` | circuit breaker trip. `spec-revalidation` is a mid-run spec edit the current plan no longer satisfies (F1.3, KAR-10.3); `plan-invalid` is 06 §3.5's second failed plan version, after the one retry; `patch-rejected` is F2.5's rule table refusing a proposed patch (06 §4.3) |
| `plan.proposed`                             | `{ version: number; planHash: PlanHash; graph: PlanGraph; by; planner?: { model; effort; tier } }` | F2.2. `planner` is 06 §6's measurement record — which model planned, at what reasoning effort, in which tier |
| `plan.validation_failed`                    | `{ version; planHash: PlanHash; by; attempt; diagnostics: PlanDiagnostic[] }`            | 06 §3.5 — validation diagnostics are **events, not exceptions**. The rejected document is not carried: no `plan` row backs it |
| `plan.patch.proposed`                       | `{ patch: PlanPatch }`                                                                   | F2.4 — **the proposal is recorded even if rejected**                                                 |
| `plan.patched`                              | `{ version; fromHash; toHash; patchId; decision: PatchDecision; proposedBy? }`            | F2.5. `proposedBy` is who *authored* the patch, which `decision.by` (who decided) cannot stand in for: 06 §7's breaker reset fires only for a human-authored one |
...
| `permission.denied`                         | `{ node; attempt; permission: PermissionLevel; method; requested: string; reason: { code; detail? }; declared?: string[] }` | F5.3/F5.4 — a mediated `fs/*`, `terminal/create` or network request DeFlow refused at the ACP boundary (09-workspace-and-safety §8.2). `requested` is the agent's own string, kept verbatim beside the resolution it was decided on; `reason` is a structured code (`path-escape` + `traversal`\|`symlink`\|`absolute`\|`invalid`, or KAR-08.7's `scope-violation`), never a message, because the node inspector renders it and KAR-08.3's gate budget counts by it. `declared` is populated only for `scope-violation`: the node's declared write globs, as its own field rather than folded into `reason.detail` |
| `permission.decided`                        | `{ node; attempt; permission: PermissionLevel; method; requested: string; resolved?: string; outcome: 'allow'\|'deny'\|'gate'; answered: 'allow'\|'reject'\|'cancelled'; by: 'ladder'\|'run-policy'\|'operator'\|'deadline'\|'withdrawn'; reason?: { code; detail? }; optionId?: string }` | F5.4/F5.6 — KAR-13.4 AC9: **every** mediated permission request and what DeFlow answered, including the ones the policy table answered by itself and no human ever saw. `permission.denied` records the refusals the agent was told about; this is the complete history NF10 asks for, which is what makes "was this agent reading files it had no business reading" answerable after the fact. `outcome` is the ladder's answer and `answered` is what went back on the wire — they differ exactly when an operator or a run-scoped `_always` overturned a gate |
| `node.scope_warning`                        | `{ node; attempt; declared: string[]; paths: string[] }`                                | F5.3/D14 — KAR-08.7's completion-time backstop: a write landed outside the node's declared `pathScope` with no preceding `permission.denied` to catch it (an adapter that never populates `ToolCallLocation`), detected by diffing the worktree. A **warning**, not a gate — declared scope is a plan-time prediction, and `git merge-tree --write-tree` (D14) is what actually gates a merge (09-workspace-and-safety §6.3) |
| `env.declared`                              | `{ node; attempt; name: string }`                                                                       | F5.6/AR-1 — a node's config named a scrubbed variable it legitimately needs (KAR-08.4, 15-security-model.md §4.1). `name` only, **never** the value — the ledger and every artifact must be scannable for the token's own value and find zero matches, so the value never becomes a field this schema could carry |
| `provider.auth_mode`                        | `{ node; attempt; provider; mode: 'subscription' \| 'api_key' }`                                        | F3.8/AR-1 — the effective auth mode a node's provider ran under, recorded so it is a fact the run report and cost report read rather than something inferred from a bill (KAR-08.8, 15-security-model.md §2.3) |
| `provider.auth_shadow_stripped`             | `{ node; attempt; provider; variable: string }`                                                         | F3.8/AR-1 — an auth-shadowing variable (`ANTHROPIC_API_KEY` and its peers) was present while the node was configured for subscription auth, so `buildChildEnv()`'s allowlist dropped it (KAR-08.4) and this is the loud record of that (KAR-08.8, 15-security-model.md §2.3). `variable` only, **never** the value, the same discipline as `env.declared` |
| `sandbox.degraded`                          | `{ node; attempt; provider; key: string; detected: string; required: string }`                           | F5.4 — a vendor sandbox setting the installed CLI is too old to understand, omitted rather than emitted (KAR-08.5, 09-workspace-and-safety §9.2). An event rather than a log line because an unknown settings key is *ignored* rather than rejected, so the operator would otherwise believe a policy is in force that is not; `key` is a settings path (`sandbox.network.strictAllowlist`), and `detected` is what DeFlow found rather than what it assumed |
...
| `workspace.worktree_reused`                 | `{ node; path; branch: string \| null; baseRef; detached: boolean; lockReason }`          | KAR-25.8 — a second `provision` for the same run and node, found already registered under this node's own lock reason. Same shape as `worktree_created` deliberately: same worktree, not a new one |
...
| `human.interjected`                         | `{ node; text; mode: 'next-turn' \| 'pause-and-inject'; delivery: 'queued' \| 'unsupported' }` | F8.2 — `delivery` is the accept-time answer, read off the adapter's probed capability row; it is never `delivered` here (KAR-13.3) |
| `human.interjection.delivered`              | `{ node; interjectedSeq: EventSeq }`                                                     | F8.5 — the receipt, appended **after** the `session/prompt` turn resolves, so a crash leaves the projection saying `queued` rather than claiming a delivery that never happened |
| `budget.consumed`                           | `{ node?; provider; usage: TokenUsage; costUsd: number }`                                | F9.1                                                                                                 |
| `budget.exceeded`                           | `{ scope: 'node' \| 'run'; node?; dimension: 'cost' \| 'wallclock'; limit; actual; failureClass: 'gate'; firedBy: 'deflow' \| 'vendor'; basis? }` | F4.6 — **pauses, does not fail**. `failureClass` is a field rather than an assumption: the schema refuses `transient` and `permanent`, so nothing downstream can retry into the same wall. `firedBy` separates DeFlow's own admission check from the vendor's ceiling below it (`--max-budget-usd`), and `basis` carries F9.1's rollup breakdown so a pause driven wholly by an estimate says so |
