...
This document states what Karvan guarantees, what it does not, and who it is defending against.
...
(PRD §5), and NF2 requires it to be *verifiable by inspection*, which means it has to be stated as
...
> **AR-1 (inviolable).** Karvan never possesses a model credential. It launches the vendor's own
...
|---|---|---|
| **Karvan reads no token file** | No code path in karvand opens a vendor credential store | All filesystem access goes through one `Fs` port; a deny-glob is asserted in tests (§1.2) |
| **Karvan sets no auth environment variable** | The child environment Karvan constructs contains no `*_API_KEY`, `*_TOKEN` or vendor auth variable it did not receive an explicit instruction to pass | One `buildChildEnv()` function, allowlist-based (§4) |
| **Karvan transmits no credential anywhere** | karvand makes no outbound network request to a model provider, ever | Karvan has no model-provider HTTP client in its dependency tree; the only outbound sockets are the OTLP exporter (user-configured, off by default) and the team hub (M3, §7) |

That AR-1 *works* was observed directly. **Verified 2026-08-02:** the Claude Code ACP adapter
...
from Karvan. That empty array is AR-1 working exactly as designed.
...
**Karvan surfaces that to the user as a shell command to run themselves, in their own terminal, and
...
The boundary is the `spawn()` call. Everything above it is Karvan's code and must be credential-free;
...
3. **A credential-path deny test.** All filesystem access in karvand goes through an `Fs` port. A
...
4. **A no-provider-SDK test.** Assert that the production dependency closure of the `karvan` package
...
Karvan probes which agent binaries are installed *and authenticated*, and plans only against what is
...
- **`mediatedExecution`.** The one capability bit that decides whether Karvan can enforce the
...
If a user chooses to supply their own key, or a company key, Karvan uses it for that provider (F3.3).
That path is explicit, per-provider, opt-in, and visible in the run manifest. It does not soften
AR-1: AR-1 is about Karvan never *appropriating* a subscription credential, not about refusing a key
...
Karvan treats this as a detectable condition and surfaces it loudly (F3.8):

- **At `karvan doctor`:** report every provider whose environment contains an auth-shadowing variable,
...
- **At run start:** if a node's provider config selects subscription auth but a shadowing variable is
  present in karvand's own environment, **strip it from the child environment** (§4) so subscription
...
The invariant is that the *effective* auth mode of every provider is a recorded, rendered fact, and
...
The PRD describes `karvand` on `http://localhost:7777` and a phone on the same Wi-Fi as a client.
...
extension, every agent Karvan itself spawned — can reach `127.0.0.1:7777`. That last one matters
most: an agent with a prompt-injected instruction and a shell is *inside* the trust boundary of an
...
| **Bearer token** | Generated at first run: 32 bytes from `crypto.randomBytes`, base64url. Stored in the repo-local, gitignored `.karvan/daemon.json` alongside `{ pid, port, startedAt }`, mode `0600`. Every HTTP and SSE request requires `Authorization: Bearer <token>`. Constant-time comparison. |
...
**Getting the token into the browser without leaking it.** `karvan up` prints a URL with the token in
...
stream is consumed with `fetch` plus a `ReadableStream` reader, which also lets Karvan send
...
| `PATH` | Resolved from the user's login shell at daemon start, not karvand's inherited `PATH` |
...
Everything else is dropped. The families that are *never* passed through implicitly:
...
- **`SSH_AUTH_SOCK` is dropped from *agent* environments but kept for Karvan's own git invocations.**
...
inherits from somewhere else, or a process the agent spawns after Karvan has stopped watching.
...
Karvan's artifacts are, by design, the most sensitive thing it produces: exact prompts, assembled
...
PRD's own analysis. Karvan has to do better, and the important decision is *where* redaction happens.

> **Redact at export, not at write.**

| | Write path | Export path |
|---|---|---|
| **What** | Everything, verbatim, content-addressed on disk under `~/.karvan` | Run report (F10.13), team-hub sync (§7), OTel export (F10.12), any clipboard or file the user shares |
...
filtering**: `~/.karvan` is created mode `0700`, per-run directories `0700`, and nothing under it is
...
allowlisting of things that only *look* like secrets — is specified in
...
| **A future Karvan contributor** | AR-1 itself | A well-meaning PR adds `process.env.ANTHROPIC_API_KEY` to a child environment "so the adapter works" | The five CI checks in §1.2 — this actor is the reason those checks exist rather than being a convention in a document |
...
- **Defending the user against themselves at `full` permission.** `full` is an explicit per-run
  ...
  honest phrasing, and Karvan says the same thing in the same words.
- **A malicious vendor CLI.** Karvan spawns first-party binaries the user already trusts with their
  code and their credentials. If that binary is hostile, nothing in this document helps.
- **Multi-tenancy.** Karvan is single-user, per-machine. The hub aggregates; it does not execute.
- **Protecting the ledger from the user's own OS account.** Everything Karvan stores is the user's,
...
When Karvan goes to a team, **runs still execute on each engineer's machine with their own seat
...
internal security review the PRD anticipates: *the hub is an event aggregator with no ability to run
anything and no credential to run it with.*
...
- **Hub authentication is separate from daemon authentication.** The daemon's bearer token is a
  ...
  scoped to publish, stored in `~/.karvan` mode `0600`, and it is on the deny-list of §4 like any
  other `*_TOKEN`.
...
| Do not | Because |
|---|---|
| Treat `127.0.0.1` as authentication | Every local process reaches it, including agents Karvan spawned, and DNS rebinding reaches it from the browser |
...
| Assume redaction protects the disk | It does not. `~/.karvan` at mode `0700` protects the disk; redaction protects exports |
| Deny-list environment variables | New credential variable names appear constantly. Allowlist, and drop everything else |
| Pass `SSH_AUTH_SOCK` to agents | It grants the agent every host the user's ssh agent can reach; Karvan's own `Git` wrapper needs it, the agent does not |
