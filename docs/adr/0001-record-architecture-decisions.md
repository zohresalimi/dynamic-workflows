...
Karvan is being built by one person, alongside a job and a degree, in an area where the ground moves
...
Six months from now the *choice* will still be visible in the code, but the *reason* will not — and
...
The architecture documents in `../` describe how Karvan works *now*. They are rewritten as the
...
1. **Every ADR carries a "Revisit when" section with a concrete, checkable trigger.** Not "when the
   ecosystem matures" but "when `node:sqlite` reaches Stability 2 *and* Node 26 is our floor". A
   trigger you cannot check is not a trigger.
2. **ADRs are immutable once accepted.** A decision that is overturned gets a *new* ADR that
...
- Superseding rather than editing makes the *history* of the architecture readable, which matters
...
- **Nothing; rely on git history and commit messages.** Rejected: commit messages record *what*
  changed, almost never *what was rejected and why*. Half the value here is the alternatives.
