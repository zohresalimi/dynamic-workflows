...
**Ship a headless local daemon (`karvand`) that serves a web UI over HTTP + SSE on
...
In development the Vite server runs *inside* the daemon rather than beside it
...
- Many clients from one stream: browser tab, phone on the same Wi-Fi, `karvan run` in a terminal,
...
- Zero packaging cost during the phase where there is one user (I6). Team rollout is
  `npx karvan up` per engineer (I5, NF6).
...
- Daemon lifecycle becomes our problem: start, stop, double-launch, orphaned children, stale
  worktree locks. Handled by an epoch counter and a `flock` on `~/.karvan/karvan.lock`
...
- **TUI as the primary surface.** Rejected: fails I3 and I4 outright. A thin `karvan` CLI for
...
- **Native desktop app (Electron/Tauri) as primary for v1.** Rejected: packaging, signing,
  ...
  to put the engine in the app process, violating I2. Revisited as an M3 *shell* over the same UI.
...
- A colleague needs to watch a run executing on someone else's machine → that is the M3 hub
  (PRD §9.5), and it aggregates *events*, never credentials or model traffic. If that ever requires
