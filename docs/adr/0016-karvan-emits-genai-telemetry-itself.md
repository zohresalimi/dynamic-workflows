# ADR 0016: Karvan emits `gen_ai.*` telemetry itself
...
for free" is only true if **Karvan** emits the standard namespace.

That is fortunate rather than unfortunate, because Karvan is better positioned to emit it anyway.
Karvan knows the plan, the node, the provider, the permission level, the patch reason and the
compaction event; the CLI knows only its own turn. And the token data needed for the spans arrives
in Karvan's hands regardless: Claude Code's result envelope carries
...
**Karvan is the emitter. It produces `gen_ai.*` OTel spans itself, over OTLP.**
...
| Karvan concept | Span | Key attributes |
|---|---|---|
...
| `gate` node | `execute_tool <gateId>` | plus `karvan.gate.verdict` |
...
Karvan's run and subgraph spans have a standard operation name. And
...
- **Everything Karvan-specific goes under a `karvan.*` namespace** — plan version, patch reason,
...
- **All of it is isolated behind one module**, `src/telemetry/semconv.ts`, exporting Karvan's own
...
### Positive
- F10.12's promise becomes true: a Karvan run is readable in Phoenix, Langfuse or Datadog with no
  vendor cooperation.
- Karvan's spans are *richer* than anything the CLIs could emit, because they carry plan and node
...
- Trace viewers are still trace viewers. They cannot show a plan graph, plan evolution, or what a
  context packet contained versus what compaction removed — which is why Karvan builds its own
...
- **Langfuse as the local backend.** Rejected on NF6: six services and a hard Docker dependency for
  a tool whose install story is `npx karvan up`. Kept for the M3 hub.
- **Skip OTel entirely and rely on Karvan's own UI.** Tempting given the instability, and it is why
...
3. **A vendor CLI starts emitting genuine `gen_ai.*` spans.** Then Karvan's spans and the CLI's would
