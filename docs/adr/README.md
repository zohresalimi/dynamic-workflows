...
Every significant architectural choice in Karvan is recorded here with the forces that produced it,
...
The architecture documents in [`../`](../README.md) describe how the system works *now*; they are
...
| [0002](./0002-headless-daemon-with-localhost-web-ui.md) | Headless daemon with a localhost web UI | Accepted | `karvand` owns execution and serves the UI on one port, 7777; the browser is just a client. |
| [0003](./0003-never-hold-provider-credentials.md) | Karvan never holds a provider credential | Accepted | AR-1. Spawn the vendor's own binary under the user's account; make "no credentials" a testable property. |
...
| [0006](./0006-journaled-dag-state-machine-not-deterministic-replay.md) | Journaled DAG state machine, not deterministic replay | Accepted | Checkpoint-and-memoise over an event ledger. Buys a karvand that is upgradeable mid-run. |
...
| [0009](./0009-pnpm-workspaces-single-published-package.md) | pnpm workspaces, one published package | Accepted | Seven private `@karvan/*` packages inlined into a single `karvan` tarball. No task runner at M1. |
...
| [0011](./0011-vite-middleware-mode-inside-the-daemon.md) | Vite in middleware mode inside karvand | Accepted | One process, one port, no proxy. Vite's dev proxy is documented-bad at SSE, and the UI *is* SSE. |
...
| [0013](./0013-delegate-sandboxing-to-vendor-clis.md) | Delegate sandboxing to the vendor CLIs | Accepted | Four layers; Karvan owns policy and mediation. Nesting your own bwrap profile breaks a working sandbox. |
...
| [0016](./0016-karvan-emits-genai-telemetry-itself.md) | Karvan emits `gen_ai.*` telemetry itself | Accepted | The PRD's assumption is contradicted — Claude Code emits a private `claude_code.*` namespace. Phoenix, not Langfuse, locally. |
...
1. **Every ADR has a "Revisit when" section with a concrete, checkable trigger.** Not "when the
   ecosystem matures" — "when `node:sqlite` reaches Stability 2 *and* Node 26 is our floor". A
