...
A Karvan run has to be legible to three different audiences, and the fastest way to ruin all three
...
longer point at a row and say *this* is why the UI shows that. Worse, it creates a standing
...
Enforce it in review with one question: *if this line vanished, would any state be wrong?* If yes,
...
export const log = pino({
  level: process.env.KARVAN_LOG_LEVEL ?? 'info',
  redact: {
    paths: ['*.authorization', '*.token', '*.headers.cookie',
            'env.ANTHROPIC_API_KEY', 'env.OPENAI_API_KEY', 'env.GITHUB_TOKEN'],
    censor: '[redacted]',
  },
})
...
## 2. The PRD is wrong about vendor telemetry — Karvan must be the emitter

PRD §4.6 states: *"Copilot, Codex and Claude Code emit GenAI-convention telemetry natively, so their
runs are readable in any OTLP backend."* The transport half is true. The **convention** half is
...
`codex mcp-server` emits nothing. `codex exec` is precisely the mode Karvan's fallback shim uses.

### Consequence (D16)

> **Karvan emits the `gen_ai.*` spans itself**, translating from each adapter's native output —
...
This is not a downgrade. Karvan is already parsing every one of those streams for the ledger, so the
...
- **Do not enable vendor telemetry on the user's behalf.** Setting `CLAUDE_CODE_ENABLE_TELEMETRY=1`
  ...
  user has already configured it, that is their pipeline and Karvan leaves it alone.
...
| Karvan concept | Span name | `gen_ai.operation.name` | Key attributes |
|---|---|---|---|
...
| `gate` node | `execute_tool <gateId>` | `execute_tool` | + `karvan.gate.verdict` |
...
for emitting from Karvan rather than inventing a shape:

| Karvan / vendor field | OTel attribute |
|---|---|
...
### Everything Karvan-specific goes under `karvan.*`
...
| Attribute | On |
|---|---|
| `karvan.run.id`, `karvan.node.id`, `karvan.node.type` | every span |
| `karvan.plan.version` | `invoke_workflow`, `invoke_agent` |
| `karvan.patch.reason`, `karvan.patch.decision` | replan spans |
| `karvan.permission.level` | `invoke_agent`, `execute_tool` |
| `karvan.gate.verdict` (`pass`\|`fail`\|`needs-human`) | gate spans |
| `karvan.compaction.scope`, `karvan.compaction.fidelity` | compaction span events |
| `karvan.adapter.kind` (`acp`\|`shim`\|`api`\|`mock`), `karvan.adapter.version` | `invoke_agent` |
| `karvan.worktree.branch` | write nodes |
...
- **Every** `gen_ai.*` attribute is **Development** stability. There is no 1.0. They live under the
  `/incubating` import subpath *precisely so* they can break without a major version bump.
...
2. **Opt in explicitly** so the SDK emits the current experimental shape rather than a legacy one:
   ...
   ```
3. **Isolate the whole surface behind one file.** `src/telemetry/semconv.ts` re-exports Karvan's own
...
/** Karvan's own namespace. Never collides with a gen_ai rename. */
export const KARVAN = {
  runId: 'karvan.run.id',
  nodeId: 'karvan.node.id',
  planVersion: 'karvan.plan.version',
  gateVerdict: 'karvan.gate.verdict',
  permissionLevel: 'karvan.permission.level',
} as const
...
core", and the whole install pitch is `npx karvan up`. A local observability backend that requires a
...
Keep Langfuse **documented** for the M3 Karvan Hub (PRD §9.5), where Docker is already assumed. Its
...
**One honest caveat:** Phoenix is a Python package. It is a *developer-machine* dependency for
inspecting traces, never a runtime dependency of `karvand`, and `npx karvan up` must work with no
...
Large payloads must not live in the ledger. This is an observability concern *and* a durability
...
a hypothetical: the single most common large artifact in Karvan is *the same failing test log,
emitted again on attempt 2 and attempt 3 of a repair loop* (F7.5). A three-attempt loop over a 20 MB
...
- **Frame cap, 8 MiB.** The ACP SDK's `LineBuffer` has **no maximum line length** — `push()`
  ...
  newline grows that buffer until karvand OOMs. Interpose a `TransformStream` upstream of
...
For scale calibration: a *trivial* `claude -p "say ok"` turn emitted a single **16,024-byte** JSON
...
| Tool | Why not |
|---|---|
| **gitleaks** | Go binary, and its value is git-*history* scanning. Karvan's artifacts are loose text blobs (`prompt.txt`, `stdout.log`, `output.json`), not commits. The `gitleaks` npm package is a stale, unrelated 1.0.0 from 2022. Shipping a Go binary breaks single-command install across three platforms (NF6). **But harvest its ruleset** — see below. |
...
| **detect-secrets** | Python, plus a baseline-file workflow designed for legacy repos with pre-existing secrets to grandfather in. Wrong shape: Karvan generates fresh artifacts every run and wants zero findings, not a maintained baseline. |
...
rules under `.karvan/redaction/rules/`, and record the provenance of each ported rule in a comment.
...
You *know* where secrets come from: `process.env` at child-spawn time. Karvan is the process that
...
covers env vars in the manifest; this covers env values that *leaked into output*, which is G14's
actual failure mode.
...
type Event =
  ...
  | { t: 'export.blocked';  runId: RunId; reason: 'scanner-error' | 'scanner-timeout'
                          | 'rules-unavailable'; detail: string }
  | { t: 'export.completed'; runId: RunId; findings: number; artifacts: number }
```

Ship `karvan redact --dry-run <runId>` so the user can preview findings before sharing, and put the
...
export function startTelemetry(cfg: Config) {
  ...
  const sdk = new NodeSDK({
    serviceName: 'karvan',
    traceExporter: new OTLPTraceExporter({ url: `${cfg.otel.endpoint}/v1/traces` }),
  })
  sdk.start()
  return sdk
}
...
That ordering is deliberate. A span is emitted *because* an event was durably written, so a span can
...
- **Do not invent `gen_ai.*` attribute names.** Use `karvan.*` for anything the convention does not
...
- **Do not treat the Claude Code internals in this document as stable.** The `claude_code.*` names,
  ...
  **one** version (2.1.220). They are private implementation details with no compatibility guarantee
  ...
  (F3.4) so drift is caught by `karvan doctor`, not by a failed three-hour run.
