...
const OTHER_SHA = `sha256-${'c'.repeat(64)}`;
const BARE_SHA = 'd'.repeat(64);
const HANDLE = `artifact://${'b'.repeat(64)}`;
const IKEY = `${RUN_ID}/${NODE}/1/0`;
const AT = '2026-08-02T14:11:33.000Z';

const usage = { inputTokens: 1200, outputTokens: 340, source: 'vendor-reported' };
...
/**
 * One representative payload per kind, in §9's order. Hand-written rather
 * than generated: a generated fixture would satisfy the schema by
 * construction and prove nothing about whether the schema says what the table
 * says.
 */
