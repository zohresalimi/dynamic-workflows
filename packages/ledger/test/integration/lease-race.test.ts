...
suite('two daemons started at the same instant (AC1, AC2)', () => {
  it('gives the lease to exactly one of them, every time', async ({ tmp }) => {
  ...
  it('tells the loser the same one sentence it tells a late arrival (AC2)', async ({ tmp }) => {
    const rounds = 36;
    ...
    for (const loser of losers) {
      ...
      expect(loser.message).not.toMatch(/EWOULDBLOCK|SQLITE_BUSY|EAGAIN|errno|database is locked/i);
    }
  });

  it('names the process that really holds it, to both the loser and a later daemon', async ({
    tmp,
  }) => {
